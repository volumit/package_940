...
#ifdef HTC_SUPPORT
  , OPTION_MEM_HOLES,
  OPTION_NO_WARN_FLAGS,
  OPTION_WARN_FLAGS,
  OPTION_WARN_ORPHAN,
  OPTION_WARN_SECTION_ASSIGNMENT,
  OPTION_EXT_MAP,
  OPTION_WARN_DISCARD_SECTION,
#ifdef HTC_SUPPORT_XML
  OPTION_XML_MAP,
#endif /* HTC_SUPPORT_XML */
#endif /* HTC_SUPPORT */
#ifdef HTC_SUPPORT
  OPTION_ERROR_NUMBERS
#endif /* HTC_SUPPORT */
};
...
static const struct ld_option ld_options[] =
{
...
#ifdef HTC_SUPPORT
  { {"mem-holes", no_argument, NULL, OPTION_MEM_HOLES},
    '\0', NULL, N_("assign input sections to the next output section if the memory region is full"),
    TWO_DASHES },
  { {"warn-flags", no_argument, NULL, OPTION_WARN_FLAGS },
    '\0', NULL, N_("Warn about incompatible section flags between input and output sections (default)"),
    TWO_DASHES },
  { {"no-warn-flags", no_argument, NULL, OPTION_NO_WARN_FLAGS },
    '\0', NULL, N_("Do not warn about incompatible section flags between input and output sections"),
    TWO_DASHES },
  { {"warn-orphan", no_argument, NULL, OPTION_WARN_ORPHAN },
    '\0', NULL, N_("Warn if there is no explizit mapping between input and output section"),
    TWO_DASHES },
  { {"warn-section-assignment", no_argument, NULL, OPTION_WARN_SECTION_ASSIGNMENT},
    '\0', NULL, N_("Warn if an input section could not be assigned to an output section because of memory region shortage"),
    TWO_DASHES },
  { {"extmap", required_argument, NULL, OPTION_EXT_MAP },
    ...
    TWO_DASHES },
    { {"warn-discard-section", no_argument, NULL, OPTION_WARN_DISCARD_SECTION},
      '\0', NULL, N_("Warn if an output section defining a symbol is discarded"),
      TWO_DASHES },
#ifdef HTC_SUPPORT_XML
  { {"XML-Map", required_argument, NULL, OPTION_XML_MAP},
    '\0', N_("FILE"), N_("Write a map file in XML format"), ONE_DASH },

#endif /* HTC_SUPPORT_XML */
#endif /* HTC_SUPPORT */
#ifdef HTC_SUPPORT
  { {"error-numbers", no_argument, NULL, OPTION_ERROR_NUMBERS },
    '\0', NULL, N_("Add unique numbers for each error and warning message."), TWO_DASHES },
#endif /* HTC_SUPPORT */
};
...
parse_args (unsigned argc, char **argv)
{
  ...
  while (1)
	...
	{
	case '?':
	  einfo (_("%P: error: unrecognized option '%s'\n"), argv[last_optind]);
	  /* Fall through.  */

	default:
	  einfo (_("%P%F: error: use the --help option for usage information\n"));
	...
	case 'a':
	  ...
	  if (strcmp (optarg, "archive") == 0)
	    ...
	    einfo (_("%P%F: error: unrecognized -a option `%s'\n"), optarg);
	  break;
	case OPTION_ASSERT:
	  ...
	  if (strcmp (optarg, "definitions") == 0)
	    ...
	    einfo (_("%P%F: error: unrecognized -assert option `%s'\n"), optarg);
	  break;
	case 'A':
	...
	case OPTION_DEMANGLE:
	  ...
	  if (optarg != NULL)
		...
		einfo (_("%F%P: error: unknown demangling style `%s'"),
		       optarg);
	...
	case 'G':
	  {
	    ...
	    if (*end)
	      einfo (_("%P%F: error: invalid number `%s'\n"), optarg);
	  }
	  break;
	case 'g':
	  /* Ignore.  */
	...
	case OPTION_UNRESOLVED_SYMBOLS:
	  if (strcmp (optarg, "ignore-all") == 0)
	    ...
	    einfo (_("%P%F: error: bad --unresolved-symbols option: %s\n"), optarg);
	  break;
	...
	case 'r':
	  if (optind == last_optind)
	    ...
	    einfo(_("%P%F: error: bad -rpath option\n"));
	...
	case OPTION_SHARED:
	  if (config.has_shared)
	    ...
	    einfo (_("%P%F: error: -shared not supported\n"));
	  break;
	case OPTION_PIE:
	  if (config.has_shared)
	    {
	    ...
	    einfo (_("%P%F: error: -pie not supported\n"));
	  break;
	...
	case OPTION_SORT_COMMON:
	  if (optarg == NULL
	    ...
	    einfo (_("%P%F: error: invalid common section sorting option: %s\n"),
		   optarg);
	  break;
	case OPTION_SORT_SECTION:
	  if (strcmp (optarg, N_("name")) == 0)
	    ...
	    einfo (_("%P%F: error: invalid section sorting option: %s\n"),
		   optarg);
	  break;
	case OPTION_STATS:
	  config.stats = TRUE;
	...
	case OPTION_SECTION_START:
	  {
	    ...
	    if (optarg2 == NULL)
	      einfo (_("%P%F: error: invalid argument to option"
		       " \"--section-start\"\n"));
	    ...
	    if ((*optarg == '\0') || (*optarg2 == '\0'))
	      einfo (_("%P%F: error: missing argument(s) to option"
		       " \"--section-start\"\n"));
	...
	case '(':
	  if (ingroup)
	    einfo (_("%P%F: error: may not nest groups (--help for usage)\n"));
	...
	case ')':
	  if (! ingroup)
	    einfo (_("%P%F: error: group ended before it began (--help for usage)\n"));
	...
	case OPTION_REDUCE_MEMORY_OVERHEADS:
	  ...
	  break;
        ...
        case OPTION_HASH_SIZE:
          ...
          break;
#ifdef HTC_SUPPORT
        case OPTION_MEM_HOLES:
            {
              config.mem_holes_flag = TRUE;
            }
          break;
        case OPTION_NO_WARN_FLAGS:
          config.warn_flags = FALSE;
          break;
        case OPTION_WARN_FLAGS:
          config.warn_flags = TRUE;
          break;
        case OPTION_WARN_ORPHAN:
          config.warn_orphan = TRUE;
          break;
        case OPTION_WARN_SECTION_ASSIGNMENT:
          config.warn_section_assignment = TRUE;
          break;
        case OPTION_EXT_MAP:
            {
          ...
          break;
        case OPTION_WARN_DISCARD_SECTION:
          command_line.warn_discard_section = TRUE;
          link_info.warn_discard_section = TRUE;
          break;
#ifdef HTC_SUPPORT_XML
        case OPTION_XML_MAP:
          config.xml_map_filename = optarg;
#endif /* HTC_SUPPORT */
#endif /* HTC_SUPPORT */
#ifdef HTC_SUPPORT
        case OPTION_ERROR_NUMBERS:
          break;
#endif /* HTC_SUPPORT */
	}
    }
...
set_section_start (char *sect, char *valstr)
{
  ...
  if (*end)
    einfo (_("%P%F: error: invalid hex number `%s'\n"), valstr);
