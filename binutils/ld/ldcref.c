  ...
  if (r == NULL)
    {
      ...
      if (r == NULL)
	einfo (_("%X%P: error: cref alloc failed: %E\n"));
      r->next = h->refs;
      h->refs = r;
      r->abfd = abfd;
      r->def = FALSE;
...
#ifdef HTC_SUPPORT
void
xml_cref (p_xml_element root)
{
  struct cref_hash_entry **csyms, **csym_fill, **csym, **csym_end;
  struct bfd_link_hash_entry *hl;
  struct cref_ref *r;

  p_xml_element cref_tab = create_xml_element (XML_E_CROSS_REFERENCE_TABLE);
  p_xml_element cref;

  add_xml_child (root, cref_tab);
  ...
  for (csym = csyms; csym < csym_end; csym++)
    {
      hl = bfd_link_hash_lookup (link_info.hash, (*csym)->root.string, FALSE,
                             FALSE, TRUE);
      if (hl == NULL)
        continue;
      ...
      if (hl->type == bfd_link_hash_defined)
        {
          ...
          if (hl->u.def.section->owner != NULL
            ...
            {
              for (r = (*csym)->refs; r != NULL; r = r->next)
        ...
        cref = create_xml_element (XML_E_CROSS_REFERENCE);
        add_xml_child (cref_tab, cref);
        add_xml_attribute (cref, XML_A_SYMBOL, (*csym)->demangled);

        for (r = (*csym)->refs; r != NULL; r = r->next)
          {
            int core_id = SEC_CORE_NUMBER_GET (r->abfd->sections->flags);
            p_xml_element file = create_xml_element (XML_E_FILE);
            add_xml_child (cref, file);
            add_xml_value (file, r->abfd->filename);
            
            if (bfd_my_archive (r->abfd) != NULL)
              add_xml_attribute (file, XML_A_ARCHIVE, bfd_get_filename (bfd_my_archive (r->abfd)));

            if (core_id > 0)
              {
                char buf[7];
                sprintf (buf, "CPU%d", core_id - 1);
                add_xml_attribute (file, XML_A_REFERENCE, buf);
              }
            else
              {
                add_xml_attribute (file, XML_A_REFERENCE, "GLOBAL");
              }
          }
    }
}
...
output_one_cref (FILE *fp, struct cref_hash_entry *h)
{
  ...
  hl = bfd_link_hash_lookup (link_info.hash, h->root.string, FALSE,
			     FALSE, TRUE);
  if (hl == NULL)
    einfo (_("%P: warning: symbol `%T' missing from main hash table\n"),
	   h->root.string);
  else
    {
...
check_reloc_refs (bfd *abfd, asection *sec, void *iarg)
{
  ...
  for (; p < pend && *p != NULL; p++)
	      ...
	      : ((*q->sym_ptr_ptr)->flags & BSF_SECTION_SYM) != 0))
	{
	  ...
	  einfo (_("%X%C: error: prohibited cross reference from %s to `%T' in %s\n"),
	  ...
	  bfd *abfd ATTRIBUTE_UNUSED)
{
  ...
  h = cref_hash_lookup (&cref_table, name, FALSE, FALSE);
  if (h == NULL)
    return NULL;
