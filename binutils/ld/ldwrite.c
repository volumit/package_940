    ...
    case lang_data_statement_enum:
        {
          ...
          /* HDP-123 warn for a data statement in a section with no contents
             and noload */
          if (!((output_section->flags & SEC_HAS_CONTENTS) != 0
                || ((output_section->flags & SEC_LOAD) != 0
                    && (output_section->flags & SEC_THREAD_LOCAL))))
            {
              if (output_section->flags & SEC_NEVER_LOAD)
                {
                  lineno = statement->data_statement.exp->type.lineno;
                  einfo(_("%S: warning: data statement in NOLOAD section\n"));
                }
              break;
            }
          ...
          if (link_order == NULL)
            einfo (_("%P%F: error: bfd_new_link_order failed\n"));
...
clone_section (bfd *abfd, asection *s, const char *name, int *count)
{
  ...
  if (bfd_family_coff (abfd) && strlen (tname) > 5)
	...
	{
	  einfo (_("%F%P: error: cannot create split section name for %s\n"), name);
  ...
  if ((sname = bfd_get_unique_section_name (abfd, tname, count)) == NULL
    ...
    {
      einfo (_("%F%P: error: clone section failed: %E\n"));
      /* Silence gcc warnings.  einfo exits, so we never reach here.  */
      return NULL;
    }
  free (tname);
...
ldwrite (void)
{
  ...
  if (!bfd_final_link (link_info.output_bfd, &link_info))
	...
	einfo (_("%F%P: error: final link failed: %E\n"));
      else
	xexit (1);
    }
}
