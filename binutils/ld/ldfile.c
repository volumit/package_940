		     ...
		     lang_input_statement_type *entry)
{
  ...
  if (entry->the_bfd == NULL)
	...
	einfo (_("%F%P: error: invalid BFD target `%s'\n"), entry->target);
      return FALSE;
    }
  ...
  if (entry->search_dirs_flag || !entry->dynamic)
      ...
      if (check != NULL)
	{
	  ...
	  if (!entry->dynamic && (entry->the_bfd->flags & DYNAMIC) != 0)
	    {
	      einfo (_("%F%P: error: attempted static link of dynamic object `%s'\n"),
		     attempt);
	      bfd_close (entry->the_bfd);
	      entry->the_bfd = NULL;
	      return FALSE;
...
ldfile_open_file (lang_input_statement_type *entry)
{
  ...
  if (! entry->search_dirs_flag)
	...
	einfo (_("%F%P: error: %s (%s): No such file: %E\n"),
	       entry->filename, entry->local_sym_name);
      else
	einfo (_("%F%P: error: %s: No such file: %E\n"), entry->local_sym_name);
    }
  else
    {
	...
	einfo (_("%F%P: error: cannot find %s\n"), entry->local_sym_name);
    }
}
  ...
  if (ldlex_input_stack == NULL)
    {
      ...
      einfo (_("%P%F: error: cannot open linker script file %s: %E\n"), name);
    }
...
ldfile_set_output_arch (const char *string, enum bfd_architecture defarch)
{
  ...
  if (arch)
    ...
    einfo (_("%P%F: error: cannot represent machine `%s'\n"), string);
}
