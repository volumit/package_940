	      ...
	      break;
            ...
            case 'f':
              /* output section flags */
                {
                  flagword flags = va_arg(arg,flagword);
                  if (flags & SEC_ALLOC)
                    fprintf(fp,"a");
                  if (flags & SEC_CODE)
                    fprintf(fp,"x");
                  ...
                  if (flags & SEC_CORE_NUMBER_MASK)
                    fprintf(fp,"c%d",SEC_CORE_NUMBER_GET(flags) - 1);
                }
              break;
#endif // HTC_SUPPORT
	    case 'l':
...
{
  einfo (_("%F%P: error: internal error %s %d\n"), file, line);
}
