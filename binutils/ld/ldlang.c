...
#ifdef HTC_SUPPORT
static void xml_input_section (p_xml_element, asection*, bfd_boolean);
static void xml_statement_list (p_xml_element, lang_statement_union_type *, lang_output_section_statement_type *);
static void xml_statements (p_xml_element);
static p_xml_element xml_curr_isection;
#endif
...
#ifdef HTC_SUPPORT
static void lang_memory_set_current(lang_memory_region_type *r, bfd_vma dot);
#endif
...
output_section_statement_table_init (void)
{
  if (!bfd_hash_table_init_n (&output_section_statement_table,
			      ...
			      61))
    einfo (_("%P%F: error: can not create hash table: %E\n"));
}
...
#if defined (HTC_SUPPORT) || !defined (HTC_SUPPORT)
  if ((*name == '<') && !strcmp (name, "<getmemreglist>"))
  ...
  for (p = lang_memory_region_list; p != NULL; p = p->next)
    {
	  ...
	  && (p->not_flags & sec_flags) == 0
          && (p->not_cores & (1 << (SEC_CORE_NUMBER_GET (sec_flags) - 1))) == 0)
	{
	  return p;
	}
    }
				      ...
				      bfd_boolean create)
{
  ...
  if (entry == NULL)
    {
      if (create)
	einfo (_("%P%F: error: failed creating section `%s': %E\n"), name);
      return NULL;
    }
...
lang_map_flags (flagword flag)
{
  ...
  if (flag & SEC_LOAD)
    minfo ("l");
#if defined(HTC_TRICORE) 
  if (flag & SEC_PCP)
    minfo("p");
#endif
  if (flag & SEC_CORE_NUMBER_MASK)
    {
      sprintf(buf,"c%d", SEC_CORE_NUMBER_GET(flag) - 1);
      minfo(buf);
    }
}
...
static void
xml_map_flags (p_xml_element root, flagword flag, bfd_boolean negated)
{
  p_xml_element attr;
  char buf[9];
  int i = 0;
  ...
  flag_dict flag_list[] = {
      {SEC_ALLOC, XML_V_ALLOC},
      {SEC_CODE, XML_V_CODE},
      {SEC_READONLY, XML_V_READONLY},
      {SEC_DATA, XML_V_DATA},
      {SEC_LOAD, XML_V_LOAD},
#ifdef HTC_TRICORE
      {SEC_PCP, XML_V_PCP},
  ...
  p_xml_element xml_flags = get_xml_child (root, XML_E_FLAGS);

  if (xml_flags == NULL)
    {
      xml_flags = create_xml_element (XML_E_FLAGS);
      add_xml_child (root, xml_flags);
    }

  while (flag_list[i].flag)
    {
      if (flag & flag_list[i].flag)
        {
          attr = create_xml_element(XML_E_FLAG);
          add_xml_value (attr, (negated ? concat ("!", flag_list[i].name, (char*) 0) : flag_list[i].name));
          add_xml_child (xml_flags, attr);
        }

      ++i;
    }

  if ((flag & SEC_CORE_NUMBER_MASK) && (! negated))
    {
      sprintf (buf, "CPU%d", SEC_CORE_NUMBER_GET(flag) - 1);
      add_xml_attribute (root, XML_A_CORE, buf);
    }
}

static void
xml_map_not_cores(p_xml_element root, unsigned char not_cores)
{
  p_xml_element xml_flags = get_xml_child (root, XML_E_FLAGS);
  unsigned char i;
      ...
      if (not_cores & (1 << i))
        {
          ...
          add_xml_value (flag, cpu_id);
          add_xml_child (xml_flags, flag);
        }
    }
}
...
const char *
lang_addr_to_memory_region(bfd_vma addr)
{
  lang_memory_region_type *m;
  for (m = lang_memory_region_list; m != NULL; m = m->next)
    {
      if ((addr >= m->origin) && (addr < (m->origin + m->length)))
        return m->name_list.name;
    }
  return "*default*";
}
...
lang_map (void)
{
...
#ifdef HTC_SUPPORT
  p_xml_element discarded_isecs = NULL;
  p_xml_element memory_config = NULL;

  p_xml_element xml_core_id = NULL;

  if (config.xml_map_file && lang_get_core_number())
    {
      xml_core_id = create_xml_element (XML_E_CORE_ID);
      add_xml_child (xml_root, xml_core_id);
      add_xml_value (xml_core_id, lang_get_core_name());
    }
#endif
...
#ifdef HTC_SUPPORT
                if (config.xml_map_file)
                  {
                    discarded_isecs = create_xml_element(XML_E_DISCARDED_INPUT_SECTIONS);
                    add_xml_child (xml_root, discarded_isecs);
                  }
#endif
...
#ifdef HTC_SUPPORT
            if (config.xml_map_file)
              xml_input_section (discarded_isecs, s, TRUE);
#endif
	  }
    }

#ifdef HTC_SUPPORT
  p_extmap_info emi = (p_extmap_info) xcalloc (1, sizeof(extmap_info));
  ...
  if (config.map_file)
    {
      ...
      fprintf (config.map_file, "%-16s %-18s %-18s %-18s %-18s %s\n",
               _("Name"), _("Origin"), _("Length"), _("Used"),_("Free"),_("Attributes"));
    }

#ifdef HTC_SUPPORT
  if (config.xml_map_file)
    {
      memory_config = create_xml_element (XML_E_MEMORY_CONFIGURATION);
      add_xml_child (xml_root, memory_config);
    }
#endif
  ...
  for (m = lang_memory_region_list; m != NULL; m = m->next)
    {
      ...
      if (config.map_file)
        {
          ...
          minfo ("0x%V", m->length);
          used = m->current - m->origin;
          minfo ("         0x%V",used);
          minfo ("         0x%V",m->length - used);
          if (m->flags || m->not_flags || m->not_cores)
            {
#ifndef BFD64
              minfo ("        ");
#endif
      ...
      if (config.xml_map_file)
        {
          p_xml_element mem_region = create_xml_element(XML_E_MEMORY_REGION);

          add_xml_attribute (mem_region, XML_A_NAME, m->name_list.name);
          add_xml_attribute_hex (mem_region, XML_A_ORIGIN, m->origin);
          add_xml_attribute_hex (mem_region, XML_A_LENGTH, m->length);
          add_xml_attribute_hex (mem_region, XML_A_USED,
                                 (m->current - m->origin));
          add_xml_attribute_hex (mem_region, XML_A_FREE,
                                 (m->length - (m->current - m->origin)));

          if (m->flags)
            {
              if (m->flags & SEC_CODE)
                add_xml_attribute (mem_region, XML_A_TYPE, "code");
              else if (m->flags & SEC_DATA)
                add_xml_attribute (mem_region, XML_A_TYPE, "data");

              xml_map_flags (mem_region, (m->flags & (~SEC_CODE) & (~SEC_DATA)),
                             FALSE);
            }

          if (m->not_flags)
            xml_map_flags (mem_region, m->not_flags, TRUE);

          if (m->not_cores)
            xml_map_not_cores(mem_region, m->not_cores);

          add_xml_child(memory_config, mem_region);
        }
#endif
    }
  ...
  if (aliases)
    {
              ...
              if (m->name_list.next)
                {
                  ...
                  for (n = m->name_list.next; n != NULL; n = n->next)
                    {
                      fprintf(config.map_file," %s",n->name);
                    }
                  print_nl ();
                }
            }
      ...
      if (config.xml_map_file)
        {
          p_xml_element list_of_aliases = create_xml_element(XML_E_MEMORY_ALIAS_LIST);
          add_xml_child (memory_config, list_of_aliases);

          for (m = lang_memory_region_list; m != NULL; m = m->next)
            {
              if (m->name_list.next)
                {
                  p_xml_element aliases = create_xml_element(XML_E_ALIASES);
                  add_xml_child (list_of_aliases, aliases);

                  lang_memory_region_name *n;

                  add_xml_attribute (aliases, XML_A_NAME, m->name_list.name);

                  for (n = m->name_list.next; n != NULL; n = n->next)
                    {
                      p_xml_element alias = create_xml_element (XML_E_ALIAS);
                      add_xml_value (alias, n->name);
                      add_xml_child (aliases, alias);
                    }
                }
            }
        }
  ...
  if (config.map_file)
    {
      extern void print_region_maps(FILE *);
      print_region_maps(config.map_file);
      int first_mirror = 1;
          ...
          if (m->mirror)
            {
              ...
              fprintf (config.map_file, "\n%-16s:", m->name_list.name);

              for (l = m->mirror; l; l = l->next)
                {
                  fprintf (config.map_file, "\n%-16s", " ");
                  for (gr = l->mirror; gr; gr = gr->next)
                    {
                      fprintf (config.map_file, "%s ", gr->region->name_list.name);
                    }
                }
            }
        }
  ...
  if (config.xml_map_file)
    {
      extern void xml_region_maps(p_xml_element);
      xml_region_maps (memory_config);
    }
#endif
	 ...
	 flagword flags)
{
  ...
  if (strcmp (s->name, DISCARD_SECTION_NAME) == 0)
    einfo (_("%P%F: error: Illegal use of `%s' section\n"), DISCARD_SECTION_NAME);
  ...
  if (s->bfd_section == NULL)
    {
      einfo (_("%P%F: error: output format %s cannot represent section called %s\n"),
  ...
  if (s->flags == 0)
    {
        extern flagword _bfd_elf_get_default_section_flags(bfd *abfd,asection *sec);
      /* there are no flags passed with this output section
         get the default section flags for this output section 
       */
      s->flags =
       _bfd_elf_get_default_section_flags(link_info.output_bfd,s->bfd_section);
    }
#endif
...
#ifdef HTC_SUPPORT
    {
      ...
      if (s->region)
        {
          new_userdata->region_name = s->region->name_list.name;
          new_userdata->region_flags = s->region->flags;
        }
		  ...
		  lang_output_section_statement_type *output)
{
  ...
  bfd_boolean discard;
#ifdef HTC_SUPPORT
  static lang_output_section_statement_type *current_output = NULL;
#endif
  ...
  if (section->output_section == NULL)
	...
	init_os (output, section, (output->flags == 0)? flags: output->flags);

#ifdef HTC_SUPPORT
      if ((flags & SEC_CORE_NUMBER_MASK)||(output->flags &
                                           SEC_CORE_NUMBER_MASK))
            {
              if ((flags & SEC_CORE_NUMBER_MASK) && (flags ^ output->flags) & SEC_CORE_NUMBER_MASK)
                {
                  if (!strcmp(section->name,output->bfd_section->name))
                  einfo("info: %B:\n input section"
                        "'%A' = '%f' not assigned to output section '%A' ="
                        "'%f'\n",
                        section->owner,
                        section,flags,
                        output->bfd_section,
                ...
                {
                  einfo("info: %B:\n input section"
                        "'%A' = '%f' assigned to output section '%A' ="
                        "'%f'\n",
                        section->owner,
                        section,flags,
                        output->bfd_section,
        ...
        if (config.mem_holes_flag && output->region)
          {
            bfd_vma dot;

            if (current_output != output)
              {
                lang_phase_type phase = expld.phase;
                enum phase_enum data_phase = expld.dataseg.phase;

                /* initialize the phase for lang_size_section */
                ...
                /* reset phase */
                expld.phase = phase;
                expld.dataseg.phase = data_phase;
              }
            dot = output->region->current;
            dot = align_power(dot,section->alignment_power);
            dot = section->size + dot;
            if (dot < output->region->origin
                || (dot - output->region->origin > output->region->length))
              {
                if (config.warn_section_assignment)
                  {
                    einfo("info: %A\nmemory region %s will overflow: %A of %B will not be assigned to %A\n",
                          output->bfd_section, 
                          output->region->name_list.name,
                          section,
                          section->owner,
                          output->bfd_section);
                  }
...
#endif
            lang_memory_set_current(output->region , dot);
          }
#endif // HTC_PPCVLE
...
#ifdef HTC_SUPPORT
      if (((output->bfd_section->flags & ~SEC_NEVER_LOAD) != 0) && config.warn_flags
              && !(output->bfd_section->flags & SEC_DEBUGGING))
        {
          flagword tmp_flags = 0;
#define FLAGS_TESTED  (SEC_READONLY | SEC_CODE | SEC_CORE_NUMBER_MASK)
          flagword test_flags =  FLAGS_TESTED;
#ifdef HTC_PPCVLE
           test_flags |=  SEC_PPC_VLE | SEC_SMALL_DATA;
#elif HTC_TRICORE
           test_flags |=  SEC_PCP | SEC_SMALL_DATA | SEC_ABSOLUTE_DATA;
#endif


          /* we allow to link section with unspecified core into every output
             section with specified core */
          if (!(flags & SEC_CORE_NUMBER_MASK))
            test_flags &= ~SEC_CORE_NUMBER_MASK;

          if ((flags ^ output->bfd_section->flags) & test_flags)
            {
              tmp_flags = output->bfd_section->flags;
              if (tmp_flags)
                {
                  einfo(_("warning: 1.incompatible section flags %B:\n input section"
                    "'%A' = '%f' \n output section '%A' ="
                    "'%f'\n"),
                    section->owner,
                    section,flags,
          ...
          else if ((flags ^ output->flags) & test_flags)
            {
              tmp_flags = output->flags;
              if (tmp_flags)
                {
                  einfo(_("warning: 2.incompatible section flags %B:\n input section"
                    "'%A' = '%f' \n output section '%A' ="
                    "'%f'\n"),
                    section->owner,
                    section,flags,
	   ...
	   asection *section)
{
  ...
  (void) section_name;  /* to suppress warning about unused variable */
	      ...
	      lang_statement_list_type *place)
{
  ...
  if (! bfd_check_format (entry->the_bfd, bfd_archive)
      ...
      if (err == bfd_error_file_ambiguously_recognized)
	{
	  ...
	  einfo (_("%B: error: file not recognized: %E\n"), entry->the_bfd);
	  ...
	  einfo ("%F\n");
	       ...
	       || place == NULL)
	einfo (_("%F%B: error: file not recognized: %E\n"), entry->the_bfd);
  ...
  switch (bfd_get_format (entry->the_bfd))
      ...
      if (entry->whole_archive)
	{
	  ...
	  for (;;)
		...
		{
		  einfo (_("%F%B: error: member %B in archive is not an object\n"),
		...
		{
		  einfo (_("%F%B: error: could not read symbols: %E\n"), member);
		  loaded = FALSE;
		}
	    }
...
open_output (const char *name)
{
  ...
  if (link_info.output_bfd == NULL)
	...
	einfo (_("%P%F: error: target %s not found\n"), output_target);

      einfo (_("%P%F: error: cannot open output file %s: %E\n"), name);
    }

  delete_output_file_on_failure = TRUE;

  if (! bfd_set_format (link_info.output_bfd, bfd_object))
    einfo (_("%P%F: error:%s: can not make object file: %E\n"), name);
  if (! bfd_set_arch_mach (link_info.output_bfd,
			   ldfile_output_architecture,
			   ldfile_output_machine))
    einfo (_("%P%F: error:%s: can not set architecture: %E\n"), name);
...
lang_track_definedness (const char *name)
{
  if (bfd_hash_lookup (&lang_definedness_table, name, TRUE, FALSE) == NULL)
    einfo (_("%P%F: error: bfd_hash_lookup failed creating symbol %s\n"), name);
}
			  ...
			  const char *name ATTRIBUTE_UNUSED)
{
  ...
  if (ret == NULL)
    einfo (_("%P%F: error: bfd_hash_allocate failed creating symbol %s\n"), name);
          ...
          switch (os->sectype)
            ...
            case noload_section:
              /* HDP-123
                 ...
                 Therefore the data statement will have no effect and will
                 be ignored.
                 All others get a noload, noalloc section. 
              */
...
process_insert_statements (void)
{
  ...
  while (*(s = &(*s)->header.next) != NULL)
      ...
      else if ((*s)->header.type == lang_insert_statement_enum)
	{
	  ...
	  if (where == NULL)
	    {
	      einfo (_("%F%P: error: %s not found for insert\n"), i->where);
	      return;
	    }
...
#ifdef HTC_SUPPORT
static void
xml_output_section_statement (p_xml_element root, lang_output_section_statement_type *output_section_statement)
{
  asection *section = output_section_statement->bfd_section;
  p_xml_element sect = create_xml_element (XML_E_OUTPUT_SECTION);

  if (output_section_statement != abs_output_section)
    {
      add_xml_child (root, sect);

      add_xml_attribute (sect, XML_A_NAME, output_section_statement->name);
      ...
      if (section != NULL)
        {
          ...
          if (output_section_statement->region)
            {
              add_xml_attribute (sect, XML_A_MEMORY_REGION ,output_section_statement->region->name_list.name);
            }

          add_xml_attribute_hex (sect, XML_A_START, section->vma);
          add_xml_attribute_dec (sect, XML_A_SIZE, section->size);

          if (section->vma != section->lma)
            {
              add_xml_attribute_hex (sect, XML_A_LOAD_ADDRESS, section->lma);
            }

          if (section->flags)
            xml_map_flags (sect, section->flags, FALSE);

          if (output_section_statement->update_dot_tree != NULL)
            exp_fold_tree (output_section_statement->update_dot_tree,
                           bfd_abs_section_ptr, &print_dot);

          xml_statement_list (sect, output_section_statement->children.head,
                              output_section_statement);
        }
    }
}
...
static void
xml_data_statement (p_xml_element root,
                    lang_data_statement_type *data)
{
  ...
  p_xml_element xml_data = create_xml_element (XML_E_DATA);

  add_xml_child(root, xml_data);
  init_opb ();
  ...
  add_xml_attribute_hex (xml_data, XML_A_ADDRESS, addr);
  add_xml_attribute_dec (xml_data, XML_A_SIZE, size);
  add_xml_attribute (xml_data, XML_A_TYPE, name);
  add_xml_attribute_hex (xml_data, XML_A_VALUE, data->value);

  if (data->exp->type.node_class != etree_value)
    {
      buf = exp_tree_to_string (data->exp);
      add_xml_attribute (xml_data, XML_A_EXPRESSION, buf);
      free (buf);
    }
...
static bfd_boolean
xml_one_symbol (struct bfd_link_hash_entry *hash_entry, void *ptr)
{
  asection *sec = (asection *) ptr;
  p_xml_element root = xml_curr_isection;

  if ((hash_entry->type == bfd_link_hash_defined
       || hash_entry->type == bfd_link_hash_defweak)
      && sec == hash_entry->u.def.section)
    {
      p_xml_element symbol = create_xml_element (XML_E_SECTION_SYMBOL);
      add_xml_child (root, symbol);

      add_xml_attribute_hex (symbol, XML_A_ADDRESS,
                             (hash_entry->u.def.value
                              + hash_entry->u.def.section->output_offset
                              + hash_entry->u.def.section->output_section->vma));

      add_xml_attribute (symbol, XML_A_NAME, hash_entry->root.string);
    }
...
xml_all_symbols (asection *sec)
{
  ...
  *ud->map_symbol_def_tail = 0;
  ...
  /* Print the symbols.  */
  for (i = 0; i < ud->map_symbol_def_count; i++)
    xml_one_symbol (entries[i], sec);

  obstack_free (&map_obstack, entries);
}

static void
xml_input_section (p_xml_element root, asection *i, bfd_boolean is_discarded)
{
  p_xml_element section = create_xml_element (XML_E_SECTION);
  p_xml_element lib;
  p_xml_element obj;
  bfd_size_type size = i->size;
  bfd_vma addr;
  ...
  add_xml_attribute (section, XML_A_NAME, i->name);
  add_xml_attribute_hex (section, XML_A_ADDRESS, addr);
  add_xml_attribute_dec (section, XML_A_SIZE, size);

  obj = create_xml_element (XML_E_OBJECT);
  add_xml_attribute (obj, XML_A_NAME, bfd_get_filename (i->owner));

  if (bfd_my_archive (i->owner) != NULL)
    {
      lib = create_xml_element (XML_E_LIBRARY);
      add_xml_attribute (lib, XML_A_NAME, bfd_get_filename (bfd_my_archive (i->owner)));
      add_xml_child (lib, obj);
      add_xml_child (section, lib);
    }
  else
    add_xml_child (section, obj);

  xml_curr_isection = section;

  if (i->output_section != NULL
      && i->output_section->owner == link_info.output_bfd)
    {
      if (link_info.reduce_memory_overheads)
        bfd_link_hash_traverse (link_info.hash, xml_one_symbol, i);
      else
        xml_all_symbols (i);
  ...
  add_xml_child(root, section);
}

static void
xml_padding_statement (p_xml_element root,
                       lang_padding_statement_type *s)
{
  bfd_vma addr;
  char *buf = (char*) xcalloc (2 * s->fill->size + 1, sizeof (char));
  char tmp[3];

  p_xml_element padding = create_xml_element (XML_E_PADDING);
  add_xml_child (root, padding);
  ...
  add_xml_attribute_hex (padding, XML_A_ADDRESS, addr);
  add_xml_attribute_dec (padding, XML_A_SIZE, s->size);
  ...
  if (s->fill->size != 0)
    {
      ...
      sprintf (buf, "%02x", *(s->fill->data));

      for (p = s->fill->data + 1, size = s->fill->size - 1; size != 0; p++, size--)
        {
          sprintf (tmp, "%02x", *p);
          buf = concat (buf, tmp, (char*) 0);
        }

      add_xml_attribute (padding, XML_A_FILL, buf);
    }
...
static void
xml_input_statement (p_xml_element root,
                     lang_input_statement_type *statm)
{
  if (statm->filename != NULL
      && (statm->the_bfd == NULL
          || (statm->the_bfd->flags & BFD_LINKER_CREATED) == 0))
    {
      p_xml_element loads = get_xml_child(root, XML_E_LOADS);
      p_xml_element file = create_xml_element (XML_E_FILE);

      /* If it is the first loaded file, we must create the Loads node. */
      if (loads == NULL)
        {
          loads = create_xml_element (XML_E_LOADS);
          add_xml_child (root, loads);
        }
      /* Avoiding duplicated nodes. */
      else
        {
          p_xml_element child = loads->children;
          while (child)
            {
              if (!strcmp (child->value, statm->filename))
                return;
              child = child->next;
            }
        }

      add_xml_value (file, statm->filename);
      add_xml_child (loads, file);
    }
}

static void
xml_reloc_statement (p_xml_element root,
                     lang_reloc_statement_type *reloc)
{
  char *buf = NULL;
  bfd_vma addr;
  bfd_size_type size;
  p_xml_element rel = create_xml_element (XML_E_RELOC);

  add_xml_child (root, rel);
  ...
  size = bfd_get_reloc_size (reloc->howto);
  buf = exp_tree_to_string (reloc->addend_exp);

  add_xml_attribute_hex (rel, XML_A_ADDRESS, addr);
  add_xml_attribute_dec (rel, XML_A_SIZE, size);
  add_xml_attribute (rel, XML_A_RELOC_NAME, reloc->howto->name);
  add_xml_attribute (rel, XML_A_NAME, (reloc->name != NULL) ? reloc->name : reloc->section->name);
  add_xml_attribute (rel, XML_A_EXPRESSION, buf);
  free (buf);
...
static void
xml_assignment (p_xml_element root,
                ...
                lang_output_section_statement_type *output_section)
{
  ...
  bfd_boolean computation_is_valid = TRUE;
  etree_type *tree;
  p_xml_element assign = create_xml_element (XML_E_ASSIGNMENT);
  char *buf = NULL;

  add_xml_child (root, assign);
  ...
  if (assignment->exp->type.node_class == etree_assert)
    ...
    {
      const char *dst = assignment->exp->assign.dst;
      add_xml_attribute (assign, XML_A_NAME, dst);
      ...
      if (computation_is_valid)
        {
          ...
          add_xml_attribute_hex (assign, XML_A_ADDRESS, value);
      ...
      else
        {
          ...
          if (h)
            ...
            add_xml_attribute (assign, XML_A_ADDRESS, XML_V_UNRESOLVED);
        }
    }
  else
    {
      add_xml_attribute (assign, XML_A_NAME, XML_V_UNDEFINED);
    }

  buf = exp_tree_to_string (assignment->exp);
  add_xml_attribute (assign, XML_A_EXPRESSION, buf);
  free (buf);
}

static void
xml_fill_statement (p_xml_element root,
                    lang_fill_statement_type *fill)
{
  size_t size;
  unsigned char *p;
  char* mask = (char*) xcalloc ((fill->fill->size * 2) + 3, sizeof(char));
  char tmp[3];

  p_xml_element xml_fill = create_xml_element (XML_E_FILL);
  add_xml_child (root, xml_fill);

  sprintf(mask, "0x");
  for (p = fill->fill->data, size = fill->fill->size; size != 0; p++, size--)
    {
      sprintf (tmp, "%02x", *p);
      mask = concat (mask, tmp, (char*) 0);
    }

  add_xml_attribute (xml_fill, XML_A_MASK, mask);
}


static void
xml_group (p_xml_element root,
           lang_group_statement_type *s,
           lang_output_section_statement_type *os)
{
  p_xml_element group = create_xml_element (XML_E_GROUP);
  add_xml_child (root, group);

  xml_statement_list (group, s->children.head, os);
}

static void
xml_address_statement (p_xml_element root,
                       lang_address_statement_type *address)
{
  char *buf = NULL;

  p_xml_element addr = create_xml_element (XML_E_ADDRESS);
  add_xml_child (root, addr);

  add_xml_attribute (addr, XML_A_SECTION, address->section_name);

  buf = exp_tree_to_string (address->address);
  add_xml_attribute (addr, XML_A_VALUE, buf);
  free (buf);
}

static void
xml_wild_statement (p_xml_element root,
                    lang_wild_statement_type *w,
                    lang_output_section_statement_type *os)
{
  struct wildcard_list *sec;

  p_xml_element wild = create_xml_element(XML_E_WILDCARD);

  add_xml_child (root, wild);

  add_xml_attribute (wild, XML_A_FILENAME, (w->filename != NULL) ? w->filename : "*");

  for (sec = w->section_list; sec; sec = sec->next)
    {
      p_xml_element pattern = create_xml_element (XML_E_PATTERN);

      add_xml_child (wild, pattern);
      if (sec->spec.exclude_name_list != NULL)
        {
          p_xml_element ex_files = create_xml_element (XML_E_EXCLUDE_FILES);
          name_list *tmp;

          add_xml_child (pattern, ex_files);

          for (tmp = sec->spec.exclude_name_list; tmp; tmp = tmp->next)
            {
              p_xml_element file = create_xml_element (XML_E_FILE);
              add_xml_child (ex_files, file);
              add_xml_value (file, tmp->name);
            }
        }

      add_xml_attribute (pattern, XML_A_VALUE, (sec->spec.name != NULL) ? sec->spec.name : "*");
    }

  xml_statement_list (wild, w->children.head, os);
}



static void
xml_statement (p_xml_element root,
               lang_statement_union_type *s,
               lang_output_section_statement_type *os)
{
  p_xml_element target;
  p_xml_element xml_output;
  p_xml_element ins;

  switch (s->header.type)
    {
    default:
      fprintf (stderr, _("Fail with %d\n"), s->header.type);
      FAIL ();
      break;
    case lang_constructors_statement_enum:
      if (constructor_list.head != NULL)
        {
          p_xml_element const_list = create_xml_element (XML_E_CONSTRUCTOR_LIST);
          add_xml_child (root, const_list);

          add_xml_attribute (const_list, XML_A_SORTED, (constructors_sorted) ? XML_V_TRUE : XML_V_FALSE);

          xml_statement_list (const_list, constructor_list.head, os);
        }
      break;
    case lang_wild_statement_enum:
      xml_wild_statement (root, &s->wild_statement, os);
      break;
    case lang_address_statement_enum:
      xml_address_statement (root, &s->address_statement);
      break;
    case lang_object_symbols_statement_enum:
      add_xml_child (root, create_xml_element (XML_E_CREATE_OBJECT_SYMBOLS));
      break;
    case lang_fill_statement_enum:
      xml_fill_statement (root, &s->fill_statement);
      break;
    case lang_data_statement_enum:
      xml_data_statement (root, &s->data_statement);
      break;
    case lang_reloc_statement_enum:
      xml_reloc_statement (root, &s->reloc_statement);
      break;
    case lang_input_section_enum:
      xml_input_section (root, s->input_section.section, FALSE);
      break;
    case lang_padding_statement_enum:
      xml_padding_statement (root, &s->padding_statement);
      break;
    case lang_output_section_statement_enum:
      xml_output_section_statement (root, &s->output_section_statement);
      break;
    case lang_assignment_statement_enum:
      xml_assignment (root, &s->assignment_statement, os);
      break;
    case lang_target_statement_enum:
      target = create_xml_element (XML_E_TARGET);
      add_xml_child (root, target);

      add_xml_value (target, s->target_statement.target);
      break;
    case lang_output_statement_enum:
      xml_output = create_xml_element(XML_E_OUTPUT);
      add_xml_child (root, xml_output);

      add_xml_attribute (xml_output, XML_A_NAME, s->output_statement.name);

      if (output_target != NULL)
        add_xml_attribute (xml_output, XML_A_FORMAT, output_target);
      break;
    case lang_input_statement_enum:
      xml_input_statement (root, &s->input_statement);
      break;
    case lang_group_statement_enum:
      xml_group (root, &s->group_statement, os);
      break;
    case lang_insert_statement_enum:
      ins = create_xml_element (XML_E_INSERT);
      add_xml_child (root, ins);

      add_xml_attribute (ins, XML_A_IS_BEFORE, 
                         (s->insert_statement.is_before) ? XML_V_TRUE : XML_V_FALSE);
      add_xml_attribute (ins, XML_A_WHERE, s->insert_statement.where);
      break;
    }
}
...
static void
xml_statement_list (p_xml_element root,
                    lang_statement_union_type *s,
                    lang_output_section_statement_type *os)
{
  while (s != NULL)
    {
      xml_statement (root, s, os);
      s = s->header.next;
    }
}

static void
xml_statements (p_xml_element root)
{
  p_xml_element section_list = create_xml_element (XML_E_SECTION_LIST);
  add_xml_child (root, section_list);

  xml_statement_list (section_list, statement_list.head, abs_output_section);
}
#endif /* HTC_SUPPORT */
...
lang_check_section_addresses (void)
{
  ...
  for (count--; count; count--)
	...
	einfo (_("%X%P: error: section %s loaded at [%V,%V] overlaps section %s loaded at [%V,%V]\n"),
  ...
  for (m = lang_memory_region_list; m; m = m->next)
    if (m->had_full_message)
      einfo (_("%X%P: error: region `%s' overflowed by %ld bytes\n"),
		 ...
		 bfd_vma base)
{
  if ((region->current < region->origin
      ...
      else if (!region->had_full_message)
	{
	  ...
	  einfo (_("%X%P: error: %B section `%s' will not fit in region `%s'\n"),
		 os->bfd_section->owner,
		 os->bfd_section->name,
		 region->name_list.name);
	}
   ...
   bfd_boolean check_regions)
{
  int dobreak = 0;
#if 0
  fprintf(stderr,"STATEMENT LIST BEGIN\n");
  dprint_statement (s, -1);
  ...
  for (; s != NULL; s = s->header.next)
	...
	case lang_output_section_statement_enum:
	  {
            /* TODO allocate section from memory region with respect to memory
               holes */
		...
		if (expld.result.valid_p)
		  dot = expld.result.value + expld.result.section->vma;
                  ...
                  {
                    lineno = os->addr_tree->type.lineno;
                    einfo (_("%F%S: error: non constant or forward reference"
	    ...
	    if (r == NULL)
	      r = lang_memory_region_lookup (DEFAULT_MEMORY_REGION, FALSE);
              ...
              {
                fat_section_userdata_type *ud;
                ud = get_userdata(os->bfd_section);
                ud->region_name = r->name_list.name;
                ud->region_flags = r->flags;
              }
		...
		bfd_vma lma = os->lma_region->current;
                ...
                if (os->lma_region != os->region)
                  {
                    /* [BIN-69] only calculate lma if the region differ,
                       otherwise vma and lma should be the same */
	    ...
	    /* Update dot in the region ?
	       ...
	       dot.  */
	    if (os->region != NULL 
		&& ((os->bfd_section->flags & (SEC_NEVER_LOAD | SEC_DEBUGGING)) == 0
		    || (os->bfd_section->flags & (SEC_ALLOC | SEC_LOAD))))
	      {
                /* if lma_region and region are in the same mirror, setting of
                   dot in region may alter the dot in lma_region */
                bfd_vma lma_current = 0;

		if (os->lma_region != NULL)
                  lma_current = os->lma_region->current;
		...
		if (os->lma_region != NULL && os->lma_region != os->region
		  ...
		  {
                    if (lma_current != os->lma_region->current)
                      {
                        /* the current of lma_region has been altered by
                           setting current of region. Set the bfd_section->lma 
                           to the new value */

                        lma_current = os->lma_region->current;
                        if (os->section_alignment != -1)
                          lma_current = align_power (lma_current, os->section_alignment);
                        os->bfd_section->lma = lma_current;
                      }
	...
	case lang_input_section_enum:
	  {
		...
		if (! bfd_relax_section (i->owner, i, &link_info, &again))
		  einfo (_("%P%F: error: can't relax section: %E\n"));
		if (again)
		  *relax = TRUE;
	      }
	    _dot = size_input_section (prev, output_section_statement,
				      output_section_statement->fill, dot);

#if 0
            if (_dot >= (output_section_statement->region->origin + output_section_statement->region->length))  
              {
                lang_statement_union_type *p;
                /* split list */
                lang_output_section_statement_type *os;

                os = (lang_output_section_statement_type *)xmalloc(sizeof(lang_output_section_statement_type));
                *os = *output_section_statement;
                os->bfd_section = 0;
                os->prev = output_section_statement;
                output_section_statement->next = os;
                if (*prev == s)
                  *prev = 0;
                else
                  {
                    for (p = *prev; p != 0 && p->header.next != s; p =
                         p->header.next);
                    if (p)
                      p->header.next = 0;
                  }
                os->children.head = s;
                output_section_statement->header.next = (union lang_statement_union *)os;
                  {
                    static int newos = 1;
                    char * cp = (char *)xmalloc(strlen(os->name)+12);
                    sprintf(cp,"%s.%d",os->name,newos);
                    newos++;
                    os->name = cp;
                  }
	        os->region = lang_memory_region_lookup ("mem1", FALSE);
                dobreak = 1;
	     //   dot = size_input_section (prev, os, os->fill, dot);
              }
            else
#endif
                dot = _dot;
		...
		else if (newdot != dot)
		  {
		    ...
		    /* If dot is advanced, this implies that the section
		       ...
		       should never be loaded.  */
                    /* HDP-123: For ELF we handle noload section like .bss.
                       If we advance the dot the section has to be allocated. */
  ...
  for (; s != NULL; s = s->header.next)
	...
	case lang_data_statement_enum:
	  ...
	  if (expld.result.valid_p)
	    ...
	    einfo (_("%F%P: error: invalid data statement\n"));
	  {
	    unsigned int size;
	    switch (s->data_statement.type)
	      {
	...
	case lang_reloc_statement_enum:
	  ...
	  if (expld.result.valid_p)
	    ...
	    einfo (_("%F%P: error: invalid reloc statement\n"));
...
lang_end (void)
{
  ...
  if (link_info.gc_sections && link_info.relocatable
    ...
    einfo (_("%P%F: error: gc-sections requires either an entry or "
	     "an undefined symbol\n"));
  ...
  if (h != NULL
	...
	einfo (_("%P%F: error:%s: can't set start address\n"), entry_symbol.name);
    }
  else
    {
      bfd_vma val;
	...
	{
	  if (! bfd_set_start_address (link_info.output_bfd, val))
	    einfo (_("%P%F: error: can't set start address\n"));
	}
      else
	{
	  asection *ts;
...
#define PPC_APUINFO_SAT 0x111
static void
check_saturation_information (bfd *input_bfd)
{
  asection *sec;
  char *contents, *p;
  bfd_size_type len;

  sec = bfd_get_section_by_name(input_bfd, ".PPC.EMB.apuinfo");
  if (sec == NULL)
    return;
  len = sec->size;
  contents = (char *) xmalloc (len);
  if (!bfd_get_section_contents (input_bfd, sec, contents, 0, len))
    einfo (_("%X%P: error: unable to read .PPC.EMB.apuinfo section contents\n"), sec);

    /* The APU section should look like this:
  ...
  */
  ...
  while (p < contents + len)
    {
      unsigned int ppc_apuinfo_ver;
      unsigned int ppc_apuinfo_num;
      if (bfd_little_endian (input_bfd))
        {
          ppc_apuinfo_ver = bfd_getl16(p);
          ppc_apuinfo_num = bfd_getl16(p+2);
        }
      else
        {
          ppc_apuinfo_num = bfd_getb16(p);
          ppc_apuinfo_ver = bfd_getb16(p+2);
        }
      if (ppc_apuinfo_num == PPC_APUINFO_SAT && (signed int)ppc_apuinfo_ver > config.max_sat_version)
        {
          einfo (_("%X%B: error: the selected cpu does not support saturation version %d\n"), input_bfd, ppc_apuinfo_ver);
        }
      p += 4;
    }
  free(contents);
  ...
  for (file = file_chain.head; file != NULL; file = file->input_statement.next)
    {
      ...
      if (config.max_sat_version != -1)
        check_saturation_information(input_bfd);
      compatible
	...
	{
	  einfo (_("%P%F: error: Relocatable linking with relocations from"
		   " format %s (%B) to format %s (%B) is not supported\n"),
		 bfd_get_target (input_bfd), input_bfd,
		 bfd_get_target (link_info.output_bfd), link_info.output_bfd);
	  /* einfo with %F: error: exits.  */
	}

      if (compatible == NULL)
	{
	  if (command_line.warn_mismatch)
	    einfo (_("%P%X: error: %s architecture of input file `%B'"
...
lang_one_common (struct bfd_link_hash_entry *h, void *info)
{
  ...
  if (!bfd_define_common_symbol (link_info.output_bfd, &link_info, h))
    einfo (_("%P%F: error: Could not define common symbol `%T': %E\n"),
	   h->root.string);
...
lang_set_flags (lang_memory_region_type *ptr, const char *flags, int invert)
{
  ...
  unsigned int core = 0;
  const char *gf = flags;
  unsigned char *ptr_not_cores = &ptr->not_cores;
	  ...
	  *ptr_flags |= SEC_PPC_VLE;
	  break;
        ...
        case 'C': case 'c' :
          ...
          else if (*ptr_flags & SEC_CORE_NUMBER_MASK)
            einfo (_("%P%F: multiple core definition in flags = %s\n"), gf);
          else
            *ptr_flags |= (core + 1) << SEC_CORE_NUMBER_SHIFT;
          break;
...
#ifdef HTC_PPCVLE
  if ((*ptr_flags & SEC_PPC_VLE) && !(*ptr_flags & SEC_CODE))
    einfo(_("%P%F: error: flags 'v' can only be set together with 'x'\n"));
#endif
  ...
  while (*flags)
   {
     switch (*flags)
      ...
      case 'C': case 'c' :
        ...
        if (sec_flags & SEC_CORE_NUMBER_MASK)
          {
            einfo (_("%P%F: multiple core definition in flags = %s\n"), gf);
            break;
          }
          ...
          {
            sec_flags |= (core) << SEC_CORE_NUMBER_SHIFT;
            core_set = 1;
          }
        break;
...
lang_process (void)
{
  ...
  if (!bfd_section_already_linked_table_init ())
    einfo (_("%P%F: error: Failed to create hash table\n"));
...
#ifdef HTC_SUPPORT
  if (config.mem_holes_flag)
    {
      /* in the case of mem_holes we have just done region allocation in 
         lang_add_section(). Do a reset before beginning real allocation
      */
      lang_reset_memory_regions();
    }
#endif
...
lang_startup (const char *name)
{
  if (startup_file != NULL)
    {
      einfo (_("%P%F: error: multiple STARTUP files\n"));
    }
		  ...
		  bfd_boolean have_vma)
{
  *lma_region = lang_memory_region_lookup (lma_memspec, FALSE);
  ...
  if (have_lma && lma_memspec != 0)
    einfo (_("%X%P: error:%S: section has both a load address and a load region\n"));
}
...
lang_record_phdrs (void)
{
  ...
  for (l = lang_phdr_list; l != NULL; l = l->next)
	   ...
	   os = os->next)
	{
	  ...
	  if (pl != NULL)
	      ...
	      if (last == NULL)
		{
		  ...
		  if (last == NULL)
		    einfo (_("%F%P: error: no sections assigned to phdrs\n"));
		}
	      pl = last;
	    }
	...
	einfo (_("%F%P: error: bfd_record_phdr failed: %E\n"));
    }
...
#if defined(HTC_SUPPORT)
void lang_add_export_symbol(const char *name, bfd_boolean type, bfd_size_type size, const char * export_name);
void lang_add_memory_region_map(const char *core, bfd_vma origin, bfd_size_type length, bfd_vma altorigin);

void
lang_add_export_symbol(const char *name, bfd_boolean type, bfd_size_type size, const char *export_name)
{
  ldemul_add_exported_symbol(name,type,size,export_name);
}
