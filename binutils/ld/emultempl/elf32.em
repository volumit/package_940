...
gld${EMULATION_NAME}_load_symbols (lang_input_statement_type *entry)
{
  ...
  if (entry->just_syms_flag
    ...
    einfo (_("%P%F: error: --just-symbols may not be used on DSO: %B\n"),
	   entry->the_bfd);
...
gld${EMULATION_NAME}_stat_needed (lang_input_statement_type *s)
{
  ...
  if (bfd_stat (s->the_bfd, &st) != 0)
    {
      einfo (_("%P:%B: warning: bfd_stat failed: %E\n"), s->the_bfd);
      return;
    }
				 ...
				 int force)
{
  ...
  if (! force)
	...
	einfo (_("%F%P:%B: error: bfd_elf_get_bfd_needed_list failed: %E\n"), abfd);
...
fragment <<EOF
  ...
  if (bfd_stat (abfd, &global_stat) != 0)
    einfo (_("%F%P:%B: error: bfd_stat failed: %E\n"), abfd);
...
fragment <<EOF
	...
	continue;
      ...
      einfo (_("%P: warning: %s, needed by %B, not found (try using -rpath or -rpath-link)\n"),
	     l->name, l->by);
    }
}
  ...
  switch (exp->type.node_class)
      ...
      if (strcmp (exp->assign.dst, ".") != 0)
	{
	  if (!bfd_elf_record_link_assignment (link_info.output_bfd,
					       ...
					       exp->assign.hidden))
	    einfo (_("%P%F: error: failed to record assignment to %s: %E\n"),
		   exp->assign.dst);
	}
...
gld${EMULATION_NAME}_before_allocation (void)
{
  ...
  if (! (bfd_elf_size_dynamic_sections
    ...
    einfo (_("%P%F: error: failed to set dynamic section sizes: %E\n"));
...
${ELF_INTERPRETER_SET_DEFAULT}
...

  {
	...
	if (! bfd_get_section_contents (is->the_bfd, s,	msg,
					(file_ptr) 0, sz))
	  einfo (_("%F%B: error: Can't read contents of section .gnu.warning: %E\n"),
		 is->the_bfd);
	msg[sz] = '\0';
  ...
  if (config.warn_orphan
      ...
      && !link_info.relocatable)
    {
      einfo (_("%P: warning: placing orphan section \"%s(%s)\"\n"),
             bfd_get_filename (s->owner), secname);
    }
#endif
         ...
         os = next_matching_output_section_statement (os, 0))
      {
        ...
        if (os->bfd_section != NULL
                        ...
                        & (SEC_LOAD | SEC_ALLOC)) == 0)))
          {
            ...
            if (lang_add_section (&os->children, s, os) == TRUE)
              {
                return os;
              }
            else
              {
                einfo(_("%X%P: error: section '%A' of %s will not fit in output section '%A' (possible memory region overflow enable --warn-section-assignment for more information or disable --mem-holes)\n"),
                      s, bfd_get_filename (s->owner), os->bfd_section);
              }
          }
      }
...
fragment <<EOF
	...
	einfo (_("%P%F: error: invalid hash style \`%s'\n"), optarg);
      break;
...
fragment <<EOF
      else if (CONST_STRNEQ (optarg, "max-page-size="))
	{
	  ...
	  if (*end || (config.maxpagesize & (config.maxpagesize - 1)) != 0)
	    einfo (_("%P%F: error: invalid maxium page size \`%s'\n"),
		   optarg + 14);
	}
      else if (CONST_STRNEQ (optarg, "common-page-size="))
	{
	  ...
	  if (*end
	    ...
	    einfo (_("%P%F: error: invalid common page size \`%s'\n"),
		   optarg + 17);
	}
...
struct ld_emulation_xfer_struct ld_${EMULATION_NAME}_emulation =
{
  ...
  ${LDEMUL_GET_CORE_NUMBER-NULL},
  ${LDEMUL_SET_CORE_NUMBER-NULL},
  ${LDEMUL_ADD_EXPORTED_SYMBOL-NULL},
  ${LDEMUL_ADD_MEMORY_MAP-NULL},
  ${LDEMUL_GET_CORE_NUMBER_FROM_NAME-NULL},
  ${LDEMUL_SET_CORE_ALIAS-NULL},
  ${LDEMUL_GET_CORE_NAME-NULL},
  ${LDEMUL_DEFINE_SDA_SECTION-NULL},
};
EOF
