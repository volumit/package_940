...
ppc_create_output_section_statements (void)
{
  ...
  if (stub_file->the_bfd == NULL
    ...
    {
      einfo (_("%F%P: error: can not create BFD %E\n"));
      return;
    }
  ...
  if (stub_file != NULL)
    {
	  ...
	  && !ppc64_elf_edit_opd (link_info.output_bfd, &link_info,
				  non_overlapping_opd))
	einfo (_("%X%P: error: can not edit %s %E\n"), "opd");
	  ...
	  && !no_tls_opt)
	{
	  ...
	  if (!ppc64_elf_tls_optimize (link_info.output_bfd, &link_info))
	    einfo (_("%X%P: error: TLS problem %E\n"));
...
ppc_add_stub_section (const char *stub_sec_name, asection *input_section)
{
 ...
 err_ret:
  einfo (_("%X%P: error: can not make stub section: %E\n"));
  return NULL;
}
  ...
  if (statement->header.type == lang_input_section_enum)
	  ...
	  && i->output_section->owner == link_info.output_bfd)
	{
	  if (!ppc64_elf_next_input_section (&link_info, i))
	    einfo (_("%X%P: error: can not size stub section: %E\n"));
	}
    }
}
...
gld${EMULATION_NAME}_finish (void)
{
  ...
  if (link_info.relocatable)
	...
	einfo (_("%X%P: error: TOC section size exceeds 64k\n"));
    }
  ...
  if (stub_added)
	...
	einfo (_("%X%P: error: can not build stubs: %E\n"));
