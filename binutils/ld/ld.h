...
typedef struct {
  ...
  /* if TRUE warn about discarded outut sections defining a symbol */
  bfd_boolean warn_discard_section;
} args_type;
...
#ifdef HTC_SUPPORT
  /* if TRUE allocate the input section to the next available output
     section if the memory area is full. */
  bfd_boolean mem_holes_flag;
  /* if TRUE warn if the input section flags are not compatible to output
     section flags. */
  bfd_boolean warn_flags;
  /* if TRUE warn about placing orphan sections */
  bfd_boolean warn_orphan;
  /* if TRUE info about not assigned section because of memroy region shortage */
  bfd_boolean warn_section_assignment;

  char *xml_map_filename;
  FILE *xml_map_file;
  signed int max_sat_version;
#endif
} ld_config_type;
...
extern const char * check_cref (const char *name, bfd *abfd);
#ifdef HTC_SUPPORT
extern void xml_cref (p_xml_element);
#endif
