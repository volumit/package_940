...
/* set to true after the first scan of the insn */
static bfd_boolean insn_scanned = FALSE;
...
#ifdef HTC_PPCVLE
static void ppc_elf_text(int);
static void ppc_apuinfo_section_add (unsigned int apu, unsigned int version);
#endif
#endif
...
#ifdef HTC_PPCVLE
bfd_boolean tf_set_mb_me(long* val1,long* val2,long value);
#endif // HTC_PPCVLE
...
/* mask to store useage of fixed register */
unsigned int ppc_fixed_regs = 0;
...

#ifdef HTC_PPCVLE
/* -mvle was passed as a commandline option */
ppc_cpu_t ppc_vle_option_passed = 0;


#ifdef DOLLAR_DOT
bfd_boolean ppcvle_dollar_dot(void)
{
  if (ppc_cpu & PPC_OPCODE_VLE)
    {
      as_bad (_("in VLE mode '$' will not be interpreted as current location"));
      return FALSE;
    }
  return TRUE;
}
...
/* This table is used for relaxing.  Each entry consists of 4 numbers:
   ...
   to the maximum possible on the PPC (+/-16MB).  However, that's
   only the maximum possible range for calls and jumps to _local_ labels
   (which thus restricts the size of an object's text segment)
   */

/* opcodes of relocatable 16-bit insns */
#define SE_B    0xe8000000      /* se_b */
#define SE_BL   0xe9000000      /* se_bl */
#define SE_BC   0xe0000000      /* se_bc */
/* the corresponding 32-bit insns */
#define E_B     0x78000000      /* e_b */
#define E_BL    0x78000001      /* e_bl */
#define E_BC    0x7a000000      /* e_bc */

/* opcodes of specail relactable insns */
#define E_ADD16I 0x1c000000     /* e_add16i */

const relax_typeS md_relax_table[] =
{
  /* BD8 -> BD15  (se_bc) */
#define RELAX_BD8     (relax_substateT) 0 
  {  254,   -256, 2, RELAX_BD8 + 1},
  {32764, -32768, 4, RELAX_BD8 + 2},
  {    1,      1, 6, 0},
#define RELAX_BD8_2      RELAX_BD8 + 3
  { 254, -256, 2, RELAX_BD8_2 + 1},
  {((1 << 24) - 2), -(1<<24), 4, 0},
};
#endif // HTC_PPCVLE
...
static const struct pd_reg pre_defined_registers[] =
{
  ...
  { "dsrr0", 574 }, /* Debug Status Save/Restore Register 0 */
  { "dsrr1", 575 }, /* Debug Status Save/Restore Register 1 */
  ...
  { "sp", 1 },
  { "SPEFSCR", 512 },	/*  LSP/EFP APU status and control register 512 R/W No No */
...
static const struct pd_reg e200zx_pre_defined_registers[] =
{
    { "BUCSR", 1013 },	/*  Branch Unit Control and Status Register 1013 R/W Yes Yes */
    { "CSRR0", 58 },    /*  Critical Save/Restore Register 0 58 R/W Yes No */
    { "CSRR1", 59 },    /*  Critical Save/Restore Register 1 59 R/W Yes No */
    { "CTR", 9 },       /*  Count Register 9 R/W No No */
    { "DAC1", 316 },	/*  Data Address Compare 1 316 R/W Yes No */
    { "DAC2", 317 },	/*  Data Address Compare 2 317 R/W Yes No */
    { "DAC3", 592 },	/*  Data Address Compare 3 592 R/W Yes Yes */
    { "DAC4", 593 },	/*  Data Address Compare 4 593 R/W Yes Yes */
    { "DBCR0", 308 },   /*  Debug Control Register 0 308 R/W Yes No */
    { "DBCR1", 309 },   /*  Debug Control Register 1 309 R/W Yes No */
    { "DBCR2", 310 },	/*  Debug Control Register 2 310 R/W Yes No */
    { "DBCR3", 561 },	/*  Debug control register 3 561 R/W Yes Yes */
    { "DBCR4", 563 },	/*  Debug control register 4 563 R/W Yes Yes */
    { "DBCR5", 564 },	/*  Debug control register 5 564 R/W Yes Yes */
    { "DBCR6", 603 },	/*  Debug control register 6 603 R/W Yes Yes */
    { "DBCR7", 596 },	/*  Debug control register 7 596 R/W Yes Yes */
    { "DBCR8", 597 },	/*  Debug control register 8 597 R/W Yes Yes */
    { "DBRAC0", 638 },	/*  same as EDBERC0 */
    { "DBSR", 304 },    /*  Debug Status Register 304 Read/Clear1 Yes No */
    { "DDAM", 576 },	/*  Debug Data Acquisition Messaging register 576 R/W No Yes */
    { "DDEAR", 600 },	/*  Debug Data Effective Address register 600 R/W Yes Yes */
    { "DEAR", 61 },     /*  Data Exception Address Register 61 R/W Yes No */
    { "DEVENT", 975 },	/*  Debug Event register 975 R/W No Yes */
    { "DMEMCFG0", 694 },/*  Local Data Memory config register 0 694 Read-only Yes Yes */
    { "DSRR0", 574 },	/*  Debug save/restore register 0 574 R/W Yes Yes */
    { "DSRR1", 575 },	/*  Debug save/restore register 1 575 R/W Yes Yes */
    { "DVC12", 318 },	/*  Data Value Compare 1 318 R/W Yes No */
    { "DVC1U2", 601 },	/*  Data Value Compare 1 Upper 601 R/W Yes No */
    { "DVC22", 319 },	/*  Data Value Compare 2 319 R/W Yes No */
    { "DVC2U2", 602 },	/*  Data Value Compare 2 Upper 602 R/W Yes No */
    { "EDBRAC0", 638 },	/*  (DBERC0) External debug resource allocation control register 0 638
                            (also temporarily aliased to 569, do not use 569) Read-only Yes Yes */
    { "ESR",  62 },     /*  Exception Syndrome Register 62 R/W Yes No */
    { "HID0", 1008 },	/*  Hardware implementation dependent reg 0 1008 R/W Yes Yes */
    { "HID1", 1009 },	/*  Hardware implementation dependent reg 1 1009 R/W Yes Yes */
    { "IAC1", 312 },	/*  Instruction Address Compare 1 312 R/W Yes No */
    { "IAC2", 313 },	/*  Instruction Address Compare 2 313 R/W Yes No */
    { "IAC3", 314 },	/*  Instruction Address Compare 3 314 R/W Yes No */
    { "IAC4", 315 },	/*  Instruction Address Compare 4 315 R/W Yes No */
    { "IAC5", 565 },	/*  Instruction Address Compare 5 565 R/W Yes Yes */
    { "IAC6", 566 },	/*  Instruction Address Compare 6 566 R/W Yes Yes */
    { "IAC7", 567 },	/*  Instruction Address Compare 7 567 R/W Yes Yes */
    { "IAC8", 568 },	/*  Instruction Address Compare 8 568 R/W Yes Yes */
    { "IMEMCFG0", 695 },/*  Local Instruction Memory config register 0 695 Read-only Yes Yes */
    { "IVPR", 63 },     /*  Interrupt Vector Prefix Register 63 R/W Yes No */
    { "L1CFG0", 515 },	/*  L1 cache config register 0 515 Read-only No Yes */
    { "L1CFG1", 516 },	/*  L1 cache config register 1 516 Read-only No Yes */
    { "L1CSR0", 1010 },	/*  L1 cache control and status register 0 1010 R/W Yes Yes */
    { "L1CSR1", 1011 },	/*  L1 cache control and status register 1 1011 R/W Yes Yes */
    { "L1FINV1", 959 },	/*  L1 cache flush and invalidate control register 1 959 R/W Yes Yes */
    { "LR",  8 },       /*  Link Register 8 R/W No No */
    { "MAS0", 624 },	/*  MPU assist register 0 624 R/W Yes Yes */
    { "MAS1", 625 },	/*  MPU assist register 1 625 R/W Yes Yes */
    { "MAS2", 626 },	/*  MPU assist register 2 626 R/W Yes Yes */
    { "MAS3", 627 },	/*  MPU assist register 3 627 R/W Yes Yes */
    { "MCAR", 573 },	/*  Machine Check Address Register 573 R/W Yes Yes */
    { "MCSR", 572 },	/*  Machine Check Syndrome Register 572 R/Clear3 Yes Yes */
    { "MCSRR0", 570 },	/*  Machine Check Save/Restore Register 0 570 R/W Yes Yes */
    { "MCSRR1", 571 },	/*  Machine Check Save/Restore Register 1 571 R/W Yes Yes */
    { "MMUCFG", 1015 },	/*  MMU/MPU configuration register 1015 Read-only Yes Yes */
    { "MPU0CFG", 692 },	/*  MPU0 configuration register 692 Read-only Yes Yes */
    { "MPU0CSR0", 1014 }, /*  MPU configuration register 1014 R/W Yes Yes */
    { "NPIDR", 517 },	/*  Nexus 3 Process ID register 517 R/W No Yes */
    { "PID0", 48 },     /*  Process ID Register 48 R/W Yes No */
    { "PIR", 286 },     /*  Processor ID Register 286 R/W Yes No */
    { "PVR", 287 },     /*  Processor Version Register 287 Read-only Yes No */
    { "SPEFSCR", 512 },	/*  LSP/EFP APU status and control register 512 R/W No No */
    { "SPRG0", 272 },   /*  SPR General 0 272 R/W Yes No */
    { "SPRG1", 273 },   /*  SPR General 1 273 R/W Yes No */
    { "SPRG2", 274 },   /*  SPR General 2 274 R/W Yes No */
    { "SPRG3", 275 },   /*  SPR General 3 275 R/W Yes No */
    { "SRR0", 26 },     /*  Save/Restore Register 0 26 R/W Yes No */
    { "SRR1", 27 },     /*  Save/Restore Register 1 27 R/W Yes No */
    { "SVR", 1023 },	/*  System Version Register 1023 Read-only Yes Yes */
    { "USPRG0", 256 },  /*  User SPR General 0  256 R/W No No */
    { "VRSAVE", 256 },  /*  USPRG0 renamed to VRSAVE in PowerISA 2.06 */
    { "XER", 1 },       /*  Integer Exception Register 1 R/W No No */
};
#define E200ZX_REG_NAME_CNT	(sizeof (e200zx_pre_defined_registers) / sizeof (struct pd_reg))
...
register_name (expressionS *expressionP)
{
  ...
  /* first scan the default predefined regs */
  reg_number = reg_name_search (pre_defined_registers, REG_NAME_CNT, name);

#ifdef HTC_PPCVLE
  if ((reg_number < 0) && (ppc_cpu & PPC_OPCODE_SPRNAMES))
    {
      /* rescan with e200zx predefined registers */
      reg_number = reg_name_search (e200zx_pre_defined_registers, E200ZX_REG_NAME_CNT, name);
    }
#endif
  ...
  *input_line_pointer = c;
  ...
  if (reg_number >= 0)
    {
      if ((ppc_fixed_regs & (1 << reg_number)) && (insn_scanned == FALSE))
        as_warn (_("fixed register r%d used as operand"), reg_number);
...
#ifdef HTC_PPCVLE
/* Opcode hash table of VLE only instructions.  */
struct hash_control *ppcvle_hash;

struct hash_control *ppcvle_transl_hash;
#endif
...
#define PPC_APUINFO_EFPU 	0x105
#define PPC_APUINFO_LSP 	0x106
#define PPC_APUINFO_ZEN420 	0x107
#define PPC_APUINFO_FIXED_REGS  0x110
...
md_parse_option (int c, char *arg)
{
  ...
  switch (c)
    ...
    case 'm':
      ...
      else if (strncmp (arg, "fixed-r",7) == 0)
        {
          int regno = atoi(arg+7);
          if ((regno > 23) || (regno < 14))
            as_bad (_("fixed register 'r%d' is not valid, has to between r14..r23"), regno);
          ppc_fixed_regs |= 1 << regno;
        }
#endif
      else
        {
...
-me200zx,               generate code for Freescale e200zx core complex\n\
-mvle			generate code for Freescale e200zx family with vle support\n\
-mz225n3,               generate code for Freescale Zen z225n3 core complex\n\
-mz410n3,               generate code for Freescale Zen z410n3 core complex\n\
-mz420n3,               generate code for Freescale Zen z420n3 core complex\n\
-mz425n3,               generate code for Freescale Zen z425n3 core complex\n\
-mz425Bn3,              generate code for Freescale Zen z425Bn3 core complex\n\
-mz710n3,               generate code for Freescale Zen z710n3 core complex\n\
-mz720n3,               generate code for Freescale Zen z720n3 core complex\n\
-mlsp			generate code for Freescale Lightweight Signal Processing APU\n\
-mefpu			generate code for Freescale Embedded Floating-Point APU revision 2\n\
-mspe			generate code for Motorola SPE instructions\n\
-msat                   generate code for Saturation instructions\n\
...
-mfixed-reg             handle 'reg' as a fixed register\n\
...
static bfd_boolean 
ppc_check_insert_opcode( 
                        struct hash_control *ppc_hash,
...
#ifdef HTC_PPCVLE
      && !(ppc_cpu & (op->deprecated & ~PPC_OPCODE_VLE)))
#else
...
ppc_setup_opcodes (void)
{
  ...
  //const ppcvle_translation* trnsl;
  //const ppcvle_translation* trnsl_end;
  const ppc_vle_macros_t* trnsl;
  const ppc_vle_macros_t* trnsl_end;
  bfd_boolean bad_insn = FALSE;
...
#ifdef HTC_PPCVLE
  if (ppcvle_hash != NULL)
    hash_die (ppcvle_hash);
  if(ppcvle_transl_hash != NULL)
    hash_die (ppcvle_transl_hash);
#endif
  ...
  for (op = powerpc_opcodes; op < op_end; op++)
    {
        if (ppc_check_insert_opcode( ppc_hash, op) == TRUE)
          bad_insn = TRUE;
    }
...
#ifdef HTC_PPCVLE
  extern const struct powerpc_opcode_table ppcvle_opcode_tables[];
  extern const int ppcvle_num_opcode_tables;
  const struct powerpc_opcode_table *opcode_table;
  const struct powerpc_opcode_table *opcode_table_end;

/*

      op_end = ppczen425_opcodes + ppczen425_num_opcodes;
  for (op = ppczen425_opcodes; op < op_end; op++)
    {
        if (ppc_check_insert_opcode( ppc_hash, op) == TRUE)
          bad_insn = TRUE;
*/
  /* insert opcode into VLE hash table */
  ppcvle_hash = hash_new ();
  ...
  for (opcode_table = ppcvle_opcode_tables; opcode_table < opcode_table_end; opcode_table++)
    {
      ...
      op_end = opcode_table->opcodes + *(opcode_table->num_opcodes);
      for (op = opcode_table->opcodes; op < op_end; op++)
        {
          //const char *retval;
      
	  if (ppc_check_insert_opcode( ppcvle_hash, op) == TRUE)
            bad_insn = TRUE;
        }
    }

  ppcvle_transl_hash = hash_new ();
  //trnsl_end = ppcvle_transl_table + ppcvle_num_transl;
  //for(trnsl = ppcvle_transl_table; trnsl < trnsl_end; trnsl++)
  trnsl_end = ppc_vle_macro_table + ppc_vle_num_macros;
  for(trnsl = ppc_vle_macro_table; trnsl < trnsl_end; trnsl++)
    {
      const char *retval;
      
	  retval = hash_insert (ppcvle_transl_hash, trnsl->name, (void *) trnsl);
	  if (retval != NULL)
        {
	      as_bad (_("duplicate translation %s"),trnsl->name);
        }
    }
#endif
...
md_begin (void)
{
  ...
  if (ppc_cpu & PPC_OPCODE_VLE)
    {
      flagword applicable;
      text_section = subseg_new(".text_vle",0);
      applicable = bfd_applicable_section_flags (stdoutput);
      bfd_set_section_flags (stdoutput, text_section,
                             applicable & (SEC_ALLOC | SEC_LOAD | SEC_RELOC
                                           | SEC_CODE | SEC_READONLY |
                                           SEC_PPC_VLE));
      ppc_vle_option_passed = PPC_OPCODE_VLE;
    }
#endif
...
{
#ifdef OBJ_ELF
  /* add the apuinfo for fixed regsiters */
  if (ppc_fixed_regs)
    ppc_apuinfo_section_add (PPC_APUINFO_FIXED_REGS, ppc_fixed_regs >> 14);
                    ...
                    unsigned int line)
{
  ...
  if ((operand->flags & PPC_OPERAND_MINPLUS1) != 0)
    min++;
...
#define MAP64(str,reloc) { str, sizeof (str) - 1, 0, 1, reloc }
  ...
  static const struct map_bfd mapping[] = {
    ...
    MAP ("ha",			BFD_RELOC_PPC_VLE_HA16A),
        ...
        if (ppc_cpu & PPC_OPCODE_VLE)
          {
            switch (reloc)
              ...
              case BFD_RELOC_HI16_S_PLTOFF:
                as_bad (_("dynamic relocations not supported in VLE mode"));
                break;
              ...
              case BFD_RELOC_PPC_TPREL16_HA:
                as_bad (_("TLS not supported in VLE mode"));
                break;
              }
          }
...
#ifdef HTC_PPCVLE
/* in VLE mode we change default .text to .text_vle */
char *
ppc_elf_check_section_name (char *name)
{
  if (ppc_vle_option_passed & PPC_OPCODE_VLE)
    {
      if (strcmp(name,".text") == 0)
        return ".text_vle";
      else if (CONST_STRNEQ (name, ".text."))
        return concat (".text_vle.", name + 6, 0);
    }
  return name;
}
static void
ppc_elf_text (int xxx)
{
  char *save_line = input_line_pointer;
  static char vle_section[] = ".text_vle\n";

  if (ppc_vle_option_passed & PPC_OPCODE_VLE)
    {
      /* Just pretend this is .section .text_vle  */
      input_line_pointer = vle_section;
      obj_elf_section (xxx);
...
#ifdef __HTC_PPCVLE__NOT_YET
/* handle insn optimization: fins a 16-bit insn to a 32-bit insn */
/* define conditions which must match to convert a 32bit into the 16bit
   instruction */
typedef enum {
  CONV_ILLEGAL,
  CONV_3REG_2VLE_REG,   /* r0,r1,r2: r0==r1 OR r0==r2 -> r0,r1 OR r0,r2 
                           r0,r1,r2 are valid VLE GPRS */
  CONV_MEMOFFSET,       /* r0,off(r1): 0 < off < 64, r0,r1 VLE GPRS */
} conv_condition_t;
...
typedef struct {
  ...
  const char *name16;
  int        num_operands;
  conv_condition_t condition;
} insn32opt_t;

insn32opt_t insn32_table [] =
{
    { "e_lwz", "se_lwz", 3, CONV_MEMOFFSET },
    { "e_add", "se_add", 3, CONV_3REG_2VLE_REG },
    { NULL, NULL, 0 }
};

extern long operand_value_powerpc(const struct powerpc_operand *, unsigned long , ppc_cpu);

static int
optimize_insn(const char *name32,
              powerpc_opcode *op32,
              powerpc_opcode **op16,
              unsigned long insn)
{
  ...
  conv = (insn32opt_t *)find_hash(ppcopt_insn,name32);
  if (opcode == (insn32opt_t *)NULL)
      return 0;

  for (i = 0; i < conv->num_operands; i++)
    operands[i] = operand_value_powerpc(op32->operands[i],insn,ppc_cpu);

  switch (conv->condition)
    {
    }
}
#endif // HTC_PPCVLE
...
md_assemble (char *str)
{
...
#ifdef HTC_PPCVLE
  opcode = (const struct powerpc_opcode *) NULL;
  if (ppc_cpu & PPC_OPCODE_VLE)
    {
      opcode = (const struct powerpc_opcode *) hash_find (ppcvle_hash, str);
      if (opcode == (const struct powerpc_opcode *) NULL)
        {
          int retval = vle_macro(str,s);
          switch(retval)
            {
            case TR_ST_OK: // instructions handled by vle_macro
              insn_scanned = FALSE;
              return;
            case TR_ST_NOTHING_TODO: // instructions not handled by vle_macro
              break;
            case TR_ST_ERROR: // invalid instruction
              insn_scanned = FALSE;
              as_bad (_("Invalid opcode in VLE mode: `%s'"), str);
              return;
            }
        }
    }
  if (opcode == (const struct powerpc_opcode *) NULL)
    {
      ...
      if (opcode && (ppc_cpu & opcode->deprecated))
        {
           as_bad (_("Invalid opcode: `%s'"), str);
           return;
        }
...
#endif		/* TE_PE */
        {
          bfd_boolean _regname;

          _regname = FALSE;
          /* only test for a register name if the operand should be a register */
          if (operand->flags & PPC_OPERAND_REG)
            { 
              _regname =  register_name (&ex);
            }
...
#ifdef HTC_PPCVLE
      if (opcode->flags & (PPC_OPCODE_VLE | PPC_OPCODE_VLE16))
        ppc_apuinfo_section_add (PPC_APUINFO_VLE, 1);
      if (opcode->flags & PPC_OPCODE_EFPU)
        ppc_apuinfo_section_add (PPC_APUINFO_EFPU, 2);
      if (opcode->flags & PPC_OPCODE_ZEN420)
        ppc_apuinfo_section_add (PPC_APUINFO_ZEN420, 1);
      if (opcode->flags & PPC_OPCODE_LSP)
        ppc_apuinfo_section_add (PPC_APUINFO_LSP, 1);
      if (opcode->flags & PPC_OPCODE_SAT)
        ppc_apuinfo_section_add (PPC_APUINFO_SAT, 1);
#endif
    }
#endif
  ...
  if (opcode->flags & PPC_OPCODE_VLE16)
    {
      unsigned int opc = insn & opcode->mask;
      int       relax = 0;
      int       r_type = 0;
      ...
      if (relax && (fc > 0))
        {
          fc = 0;
          dwarf2_emit_insn (0);
          f = frag_var(rs_machine_dependent, 6,
                       2, r_type,fixups[0].exp.X_add_symbol,
                       fixups[0].exp.X_add_number,0);
          fixups[0].reloc = reloc;
          md_number_to_chars (f, insn >> 16, 2);
...
#ifdef HTC_PPCVLE
          opflags = powerpc_operands[fixups[i].opindex].flags;
          switch (fixups[i].reloc)
            {
            default:
              break;
            case BFD_RELOC_LO16:
              if (opflags & PPC_OPERAND_VLE_I16A)
                fixups[i].reloc = BFD_RELOC_PPC_VLE_LO16D;
              else if (opflags & PPC_OPERAND_VLE_I16L)
                fixups[i].reloc = BFD_RELOC_PPC_VLE_LO16A;
              break;
            case BFD_RELOC_HI16:
              if (opflags & PPC_OPERAND_VLE_I16A)
                fixups[i].reloc = BFD_RELOC_PPC_VLE_HI16D;
              else if (opflags & PPC_OPERAND_VLE_I16L)
                fixups[i].reloc = BFD_RELOC_PPC_VLE_HI16A;
              break;
            case BFD_RELOC_HI16_S:
              if (opflags & PPC_OPERAND_VLE_I16A)
                fixups[i].reloc = BFD_RELOC_PPC_VLE_HA16D;
              else if (opflags & PPC_OPERAND_VLE_I16L)
                fixups[i].reloc = BFD_RELOC_PPC_VLE_HA16A;
              break;
            case BFD_RELOC_PPC_EMB_SDA21:
              /* we have to handle e_addi16 in a specail way, because of the 
                 special meaning of r0 as a base register */
              if ((insn & opcode->mask) == E_ADD16I)
                fixups[i].reloc = BFD_RELOC_PPC_VLE_SDA21;
              break;
            }
#endif // HTC_PPCVLE
...
ppc_macro (char *str, const struct powerpc_macro *macro)
{
  ...
  if (count != macro->operands)
    {
      as_warn ("opcode: '%s'",str);
      as_bad (_("wrong number of operands %d <=> %d (%s)"),count,macro->operands,macro->name);
      return;
    }
...
ppc_section_letter (int letter, char **ptr_msg)
{
  ...
  *ptr_msg = _("Bad .section directive: want a,e,s,w,x,M,S,G,T in string");
  return -1;
}
...
ppc_section_flags (flagword flags, bfd_vma attr, int type)
{
  ...
  if (flags & SEC_PPC_VLE)
    {
      new_cpu = (ppc_cpu & ~PPC_OPCODE_CLASSIC) | PPC_OPCODE_VLE | PPC_OPCODE_BOOKE;
    }
  else
    {
      new_cpu = ppc_cpu & ~PPC_OPCODE_VLE;
      new_cpu |= ppc_vle_option_passed;
    }

  if ((flags & SEC_CODE) && (ppc_cpu & PPC_OPCODE_VLE))
    flags |= SEC_PPC_VLE;
...
#ifdef HTC_PPCVLE
void
ppc_elf_frag_check(fragS *fragP)
{
  if (fragP->has_code)
    {
      if ((ppc_cpu & PPC_OPCODE_VLE)
      ...
      else if (((fragP->fr_address + fragP->insn_addr) & 1) != 0)
        as_bad_where (fragP->fr_file, fragP->fr_line,
...
{
  if (show_internals)
    printf ("*** md_estimate_size_before_relax (%lx): ", (valueT) ((fragP)->fr_opcode));
                 ...
                 fragS *fragP )
{
  ...
  unsigned int  insn;
  offsetT       fix_where = fragP->fr_fix;
  fixS  *new_fix;
  ...
  if (convert_insn == 0)
  ...
  else if (convert_insn == 1)
    {
      ...
      if (show_internals)
        printf("convert 16-bit to 32-bit insn\n");
      
      switch (fragP->fr_subtype)
        {
        case RELAX_BD8+1:
          if ((insn & 0xf8000000)  == SE_BC)            /* se_bc */
            {
              unsigned int new_insn = E_BC;             /* e_bc */

              new_insn |= (insn & 0x4000000) >> 6;      /* BO16 -> BO32 */
              new_insn |= (insn & 0x3000000) >> 8;      /* BI16 -> BI32 */
              md_number_to_chars(buf, new_insn, 4);
            }
          else
            {
              as_bad(_("wrong relax type %d for insn 0x%x"),
                     fragP->fr_subtype,insn);
            }
          break;
        case RELAX_BD8_2 + 1:
          if ((insn & 0xfc000000)  == SE_B)             /* se_b/sb_bl */
            {
              unsigned new_insn = E_B;                  /* e_b/e_bl */
              new_insn |= (insn & 0x1000000) >> 24;     /* set LK */
              md_number_to_chars(buf,new_insn,4);
            }
          else
            {
          ...
          break;
        default:
          as_bad(_("insn cannot be relaxed yet 0x%x"),insn);
          return;
        }
    }
  else if (convert_insn == 2)
    {
      /* expand a 16-bit jump instructions by converting the condition code
         and insert a branch insn */
      if (show_internals)
          ...
          if ((insn & 0xf8000000)  == SE_BC)            /* se_bc */
            {
              ...
              /* convert the condition BO16 */
              new_insn = insn & ~0x4000000;     /* out mask BO16 */
              new_insn |= (insn & 0x4000000) ^ 0x4000000; /* convert the
                                                           condition */
              new_insn |= 0x30000;            /* jump over the inserted branch */
              md_number_to_chars(buf,new_insn>>16,2);
              md_number_to_chars(buf+2,E_B,4);  /* insert hte jump insn */
              fix_where += 2;
              break;
            }
        default:
    ...
    as_bad("insn cannot be relaxed yet 0x%x",(unsigned int)bfd_getb32(buf));

  fragP->fr_var = 0;
  new_fix = fix_new(fragP,
                    fix_where,
                    md_relax_table[fragP->fr_subtype].rlx_length,
                    fragP->fr_symbol,
  ...
  if ((ppc_cpu & PPC_OPCODE_VLE) != 0)
    {
      if ((count != 0) && ((count & 1) == 0))
        {
          /* in vle mode we use a 2 byte nop pattern  se_or r0,r0 */
  ...
  if (count != 0 && (count & 3) == 0)
    {
      ...
      if ((ppc_cpu & PPC_OPCODE_VLE) != 0)
        {
          /* in VLE mode we use e_ori 0,0,0 */
          md_number_to_chars (dest, 0x1800d000, 4);
        }
      else
  ...
  else
    {
      ...
      switch (fixP->fx_r_type)
        ...
        case BFD_RELOC_PPC_VLE_HA16D:
            {
              char *where = fixP->fx_frag->fr_literal + fixP->fx_where;
              unsigned long insn, mask = 0;
              valueT  tmp_val = value;
              ...
              switch (fixP->fx_r_type)
                ...
                case BFD_RELOC_PPC_VLE_HI16A:
                case BFD_RELOC_PPC_VLE_HI16D:
                  tmp_val = ((tmp_val >> 16) & 0xffff);
                  break;
                case BFD_RELOC_PPC_VLE_HA16A:
                case BFD_RELOC_PPC_VLE_HA16D:
                  tmp_val = ((tmp_val + 0x8000) >> 16) & 0xffff;
                  break;
                }
              switch (fixP->fx_r_type)
                {
                ...
                case BFD_RELOC_PPC_VLE_HA16A:
                  /* use split16a */
                  mask = 0x001f07ff;
                  tmp_val = ((tmp_val & 0xf800) << 5) | (tmp_val & 0x7ff);
                  break;
                case BFD_RELOC_PPC_VLE_LO16D:
                case BFD_RELOC_PPC_VLE_HI16D:
                case BFD_RELOC_PPC_VLE_HA16D:
                  /* use split16d */
                  mask = 0x03e007ff;
                  tmp_val = ((tmp_val & 0xf800) << 10) | (tmp_val & 0x7ff);
                  break;
                }
              insn &= ~mask;
              insn |= tmp_val;
...
#ifdef HTC_PPCVLE
static char * tf_output(const char* instr,
                      ppcvle_operand ops[],
                      int numops)
{
  int size = 0;
      ...
      if(*sc == '%')
        {
          ...
          if (i < 10)
            {
              used_ops[i] = *(sc) - '0';
              if (used_ops[i] >= numops)
                {
                  as_bad(_("illegal number of operands in %s"),instr);
                  return NULL;
                }
              size+= strlen(ops[used_ops[i]].descriptor);
            }
          else
            {
              as_bad(_("illegal number of operands in %s"),instr);
  ...
  while(*sc != '\0')
    {
      if(*sc != '%')
      ...
      else
        {
          ...
          switch (*sc)
            {
            case 'b':           /* operand is a valid BO16 modify the value */
              if((ops[used_ops[i]].value & 0x1c) == 4)
                  *dc++ = '0';
              else if((ops[used_ops[i]].value & 0x1c) == 0xc)
                  *dc++ = '1';
              sc++;
              break;
            case 'B':           /* operand is a valid BO32 modify the value */
              if((ops[used_ops[i]].value & 0x1c) == 4)
                  *dc++ = '0';
              else if((ops[used_ops[i]].value & 0x1c) == 0xc)
                  *dc++ = '1';
              else if( (ops[used_ops[i]].value & 0x16)== 0x10)
                  *dc++ = '2';
              else if( (ops[used_ops[i]].value & 0x16)== 0x12)
                  *dc++ = '3';
              sc++;
              break;
            default:
              strcpy(dc,ops[used_ops[i]].descriptor);
              dc += strlen(ops[used_ops[i]].descriptor);
              break;
            }
          sc++;
          i++;
...
int vle_macro(const char* instr, char* inoperands)
{
  int i = 0;
  int j = 0;
  ppc_vle_macros_t* trans;
  struct powerpc_opcode* opcode;
  int numops = 0;
  ppcvle_operand* outoperands = 0;
  int skip_optional = 0;
  unsigned char* opindex_ptr = 0;
  char* str = strdup(inoperands);
  char* origstr = str;
  char* orig_input_line_pointer = input_line_pointer;
  char* myinstr = strdup(instr);
  char me_desc[4];
  char mb_desc[4];

  char val_to_desc[4][8];
  int currvaltodesc = 0;
  
  
  /* eliminate '-' and '+' in branches */
  i = strlen(myinstr);
  if (*myinstr == 'b')
    {
      if ((myinstr[i-1] == '-') || (myinstr[i-1] == '+'))
        myinstr[i-1] = 0;
    }
  i = 0;
  trans = hash_find(ppcvle_transl_hash,myinstr);
  opcode = hash_find(ppc_hash,myinstr);
  
  free (myinstr);
  if(trans == NULL)
    {
      return TR_ST_NOTHING_TODO; // instruction mapped to itself
    }

  if(opcode == 0)
    {
      return TR_ST_ERROR; // instruction not valid
    }
  ...
  outoperands = (ppcvle_operand*)malloc(numops*sizeof(ppcvle_operand));
  int currentOp = 0;    
  int need_paren = 0;
  int next_opindex = 0;
  ...
  for (opindex_ptr = opcode->operands; *opindex_ptr != 0; opindex_ptr++)
    {
      ...
      if(currentOp >= numops)
        {
          as_bad(_("illegal number of operands"));
          free(outoperands);
          free(origstr);
          return TR_ST_ERROR;
        }
      ...
      if ((operand->flags & PPC_OPERAND_OPTIONAL) != 0
          && skip_optional)
        {
          if (operand->flags & PPC_OPERAND_CR)
            {
              outoperands[currentOp].descriptor = "cr0";
              outoperands[currentOp].type = TR_OT_REG;
            }
          else
            {
              outoperands[currentOp].descriptor = "0";
              outoperands[currentOp].type = TR_OT_IMM;
            }
          outoperands[currentOp].value = 0;
          if ((operand->flags & PPC_OPERAND_NEXT) != 0)
          {
              next_opindex = *opindex_ptr + 1;
          }
      ...
      outoperands[currentOp].descriptor = str;
      input_line_pointer = str;
      ...
      str = input_line_pointer;
      input_line_pointer = orig_input_line_pointer;
      ...
      if (ex.X_op == O_illegal)
        ...
        {
          outoperands[currentOp].type = TR_OT_REG;
          outoperands[currentOp].value = ex.X_add_number;
        }
      else if (ex.X_op == O_constant)
        {
#ifdef OBJ_ELF
        ...
        {
          outoperands[currentOp].type = TR_OT_SYM;
        }
#endif /* OBJ_ELF */

      if((operand->flags & PPC_OPERAND_VLE_MBE) != 0)
      {
          long val1,val2;
          if(outoperands[currentOp].type != TR_OT_IMM)
          {
              as_bad (_("non immediate operand in %s"),instr);
              return TR_ST_ERROR;
          }

          if(!tf_set_mb_me(&val1,&val2,outoperands[currentOp].value))
          {
              as_bad (_("invalid mask operand in %s"),instr);
              return TR_ST_ERROR;
          }

          sprintf(mb_desc,"%2ld",val1);
          sprintf(me_desc,"%2ld",val2);
          outoperands[currentOp-1].descriptor = mb_desc;
          outoperands[currentOp-1].value = val1;
          outoperands[currentOp].descriptor = me_desc;
          outoperands[currentOp].value = val2;
          outoperands[currentOp].type = TR_OT_IMM;
      }
  ...
  /* set insn to scanned to avoid multiple warnings */
  insn_scanned = TRUE;

  numops = currentOp; /* count the realy passed operands */
  
  for (i = 0; i < trans->numoftrans; i++)
    {
      for (j = 0; j < numops; j++)
        {
          if (!ppcvle_is_op_valid(trans->trans[i].op_const[j],&outoperands[j]))
            break;
        }
      if (j != numops)
        continue;
      if (trans->trans[i].insn_const == OPS_ISVALID)
        break;
      if (ppcvle_is_op_list_valid(trans->trans[i].insn_const,
                                (trans->trans[i].op0 < 0)? 0: &outoperands[trans->trans[i].op0],
                                (trans->trans[i].op1 < 0)? 0: &outoperands[trans->trans[i].op1],
                                (trans->trans[i].op2 < 0)? 0: &outoperands[trans->trans[i].op2],
                                (trans->trans[i].op3 < 0)? 0: &outoperands[trans->trans[i].op3],
                                (trans->trans[i].op4 < 0)? 0: &outoperands[trans->trans[i].op4]))
          break;
    }
  if (i == trans->numoftrans)
    {
      free(outoperands);
      free(origstr);
      return trans->state;
  ...
  for(j=0;j<numops; j++)
    {
      if(outoperands[j].descriptor == NULL)
        {
          sprintf(val_to_desc[currvaltodesc],"%ld",outoperands[j].value);
          outoperands[j].descriptor = val_to_desc[currvaltodesc];
          currvaltodesc++;
        }
    }

  for (j = 0; j < trans->trans[i].num_trans; j++)
    {
      char *newinsn = tf_output(trans->trans[i].insns[j],outoperands,numops);
      if (newinsn == NULL)
        break;
      insn_scanned = TRUE;
      md_assemble(newinsn);
     ...
     int verbose;
{
  ...
  fprintf (stream, _("GNU assembler %s (%s)"),
           BFD_VERSION_STRING, TARGET_ALIAS);
