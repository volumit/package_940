...
//#undef HTC_SUPPORT
#if defined(HTC_SUPPORT) && defined(HTC_TRICORE)
  ...
  if (FRAG_NOTICE (frag) == (char *) NULL)
  ...
  else
    {
      ...
      sprintf (buf + strlen (FRAG_NOTICE (frag)), "\n%*s%s",
               (int)strlen (_NOTICESTRING), " ", notice);
    }
	     ...
	     char *string, unsigned int address)
{
  ...
  if (address == ~(unsigned int) 0)
	...
	fprintf (list_file, " ");
      ...
      emit_line (NULL, "\t%s\n", string ? string : "");
#if defined(HTC_SUPPORT) && defined(HTC_TRICORE)
      PERFORM_EXT_LISTING (list, list_file, on_page);
      if (list->message)
        {
          fprintf (list_file, "****  %s\n", list->message);
          listing_page (list);
...
listing_listing (char *name ATTRIBUTE_UNUSED)
{
  ...
  (void) message; /* to suppress warning about unused variable */
  while (list)
    {
