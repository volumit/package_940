...
#ifdef __HTC_SUPPORT
              /*if defined in different cores its OK */
              if (h->type == bfd_link_hash_defined
                  && ((msec->flags & SEC_TRICORE_CORE_NUMBER) !=
                      (section->flags & SEC_TRICORE_CORE_NUMBER)))
                break;
#endif
      ...
      || h->type == bfd_link_hash_defweak)
    {
      ...
      if (s != NULL
          ...
          && bfd_section_removed_from_list (obfd, s->output_section))
        {
          ...
          if (info->callbacks->warn_fix_symbols)
            (*info->callbacks->warn_fix_symbols) (info, obfd, h->root.string, s, op);
...
void
_bfd_fix_excluded_sec_syms (bfd *obfd ATTRIBUTE_UNUSED, struct bfd_link_info *info)
{
