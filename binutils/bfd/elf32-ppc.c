...
static bfd_boolean ppc_final_fixed_reg_check(struct bfd_link_info *link_info);
...
#ifdef HTC_PPCVLE
/* Some VLE instructions.  */
#define E_B		0x78000000
#define SE_B		0x0000E800
#endif /* HTC_PPCLVE */
/* Some instructions.  */
...
static reloc_howto_type ppc_elf_howto_raw[] = {
...
#ifdef HTC_PPCVLE
  /* PC 8-Bit relative displacement (S + A - P) >> 1 (format bdh8) */
  ...
  /* PC 15-Bit relative displacement (S + A - P) >> 1 (format bdh15) */
  HOWTO (R_PPC_VLE_REL15,	/* type */
	 ...
	 complain_overflow_signed, /* complain_on_overflow */
	 bfd_elf_generic_reloc,	/* special_function */
	 "R_PPC_VLE_REL16",	/* name */
	 FALSE,			/* partial_inplace */
	 0,			/* src_mask */
	 0xfffe,		/* dst_mask */
	 TRUE), 		/* pcrel_offset */

  /* PC 24-Bit relative displacement (S + A - P) >> 1 (format bdh24) */
  ...
  /* A 16 bit relocation without overflow. #lo(S+A) (format split16a)  */
  ...
  /* A 16 bit relocation without overflow. #lo(S+A) (format split16d)  */
  ...
  /* A 16 bit relocation without overflow. #hi(S+A) (format split16a)  */
  ...
  /* A 16 bit relocation without overflow. #hi(S+A) (format split16d)  */
  ...
  /* A 16 bit relocation without overflow. #ha(S+A) (format split16a)  */
  ...
  /* A 16 bit relocation without overflow. #ha(S+A) (format split16d)  */
  ...
  /* A 20 bit relocation without overflow. SDA_BASE || (X+A) (format split20)  */
  ...
  /* A 20 bit relocation without overflow. SDA_BASE || #lo(X+A) (format split20)  */
  ...
  /* A 16 bit relocation SDA relative  #lo(X+A) (format split16a)  */
  ...
  /* A 16 bit relocation SDA relative  #lo(X+A) (format split16d)  */
  ...
  /* A 16 bit relocation SDA relative  #hi(X+A) (format split16a)  */
  ...
  /* A 16 bit relocation SDA relative  #hi(X+A) (format split16d)  */
  ...
  /* A 16 bit relocation SDA relative adjusted  #ha(X+A) (format split16a)  */
  ...
  /* A 16 bit relocation SDA relative adjusted  #ha(X+A) (format split16d)  */
			   ...
			   bfd_reloc_code_real_type code)
{
  ...
  switch (code)
    ...
    case BFD_RELOC_PPC_VLE_SDAREL_LO16A:r = R_PPC_VLE_SDAREL_LO16A;     break;
    case BFD_RELOC_PPC_VLE_SDAREL_LO16D:r = R_PPC_VLE_SDAREL_LO16D;     break;
    case BFD_RELOC_PPC_VLE_SDAREL_HI16A:r = R_PPC_VLE_SDAREL_HI16A;     break;
    case BFD_RELOC_PPC_VLE_SDAREL_HI16D:r = R_PPC_VLE_SDAREL_HI16D;     break;
    case BFD_RELOC_PPC_VLE_SDAREL_HA16A:r = R_PPC_VLE_SDAREL_HA16A;     break;
    case BFD_RELOC_PPC_VLE_SDAREL_HA16D:r = R_PPC_VLE_SDAREL_HA16D;     break;
#endif
...
#ifdef HTC_PPCVLE
  if (asect->flags & SEC_PPC_VLE)
    shdr->sh_flags |= SHF_PPC_VLE;

  if (asect->flags & SEC_SMALL_DATA)
    shdr->sh_flags |= SHF_SMALL_DATA;
#endif

  return htc_elf32_fake_sections(abfd,shdr,asect);
}
...
{
  if ((hdr->sh_flags & SHF_PPC_VLE) != 0)
    {
        *flags |= SEC_PPC_VLE;
    }
  if ((hdr->sh_flags & SHF_SMALL_DATA) != 0)
    {
...
/* modify program header flags if we load a VLE code section */
static bfd_boolean
ppc_elf_modify_segment_map (bfd *abfd, struct bfd_link_info *info ATTRIBUTE_UNUSED)
{
  ...
  for (m = elf_tdata (abfd)->segment_map; m != NULL; m = m->next)
	...
	for (i = 0; i < m->count; i++)
	  {
	    if (m->sections[i]->flags & SEC_PPC_VLE)
	      m->p_flags |= PF_PPC_VLE;
	  }
      }
...
static const struct bfd_elf_special_section ppc_elf_special_sections[] =
{
  ...
  { STRING_COMMA_LEN (".sbss14"),          0, SHT_NOBITS,   SHF_ALLOC + SHF_WRITE + SHF_SMALL_DATA},
  { STRING_COMMA_LEN (".sdata14"),         0, SHT_PROGBITS, SHF_ALLOC + SHF_WRITE + SHF_SMALL_DATA},
  { STRING_COMMA_LEN (".sbss15"),          0, SHT_NOBITS,   SHF_ALLOC + SHF_WRITE + SHF_SMALL_DATA},
  { STRING_COMMA_LEN (".sdata15"),         0, SHT_PROGBITS, SHF_ALLOC + SHF_WRITE + SHF_SMALL_DATA},
  { STRING_COMMA_LEN (".sbss16"),          0, SHT_NOBITS,   SHF_ALLOC + SHF_WRITE + SHF_SMALL_DATA},
  { STRING_COMMA_LEN (".sdata16"),         0, SHT_PROGBITS, SHF_ALLOC + SHF_WRITE + SHF_SMALL_DATA},
  { STRING_COMMA_LEN (".sbss17"),          0, SHT_NOBITS,   SHF_ALLOC + SHF_WRITE + SHF_SMALL_DATA},
  { STRING_COMMA_LEN (".sdata17"),         0, SHT_PROGBITS, SHF_ALLOC + SHF_WRITE + SHF_SMALL_DATA},
#else
  { STRING_COMMA_LEN (".sbss"),           -2, SHT_NOBITS,   SHF_ALLOC + SHF_WRITE },
  { STRING_COMMA_LEN (".sbss2"),          -2, SHT_NOBITS,   SHF_ALLOC + SHF_WRITE },
  { STRING_COMMA_LEN (".sdata"),          -2, SHT_PROGBITS, SHF_ALLOC + SHF_WRITE },
  { STRING_COMMA_LEN (".sdata2"),         -2, SHT_PROGBITS, SHF_ALLOC + SHF_WRITE },
#endif
  { STRING_COMMA_LEN (".tags"),            0, SHT_ORDERED,  SHF_ALLOC },
  { STRING_COMMA_LEN (".PPC.EMB.apuinfo"), 0, SHT_NOTE,     0 },
  { STRING_COMMA_LEN (".PPC.EMB.sbss0"),   0, SHT_NOBITS,   SHF_ALLOC + SHF_WRITE + SHF_SMALL_DATA},
  { STRING_COMMA_LEN (".PPC.EMB.sdata0"),  0, SHT_PROGBITS, SHF_ALLOC + SHF_WRITE + SHF_SMALL_DATA},
#ifdef HTC_PPCVLE
  { STRING_COMMA_LEN (".text_vle"),       -2, SHT_PROGBITS,   SHF_ALLOC + SHF_EXECINSTR + SHF_PPC_VLE},
  { STRING_COMMA_LEN (".init_vle"),       -2, SHT_PROGBITS,   SHF_ALLOC + SHF_EXECINSTR + SHF_PPC_VLE},
  { STRING_COMMA_LEN (".fini_vle"),       -2, SHT_PROGBITS,   SHF_ALLOC + SHF_EXECINSTR + SHF_PPC_VLE},
  { STRING_COMMA_LEN (".ctors"),           0, SHT_PROGBITS, SHF_ALLOC},
  { STRING_COMMA_LEN (".dtors"),           0, SHT_PROGBITS, SHF_ALLOC},
#endif
  { NULL,                              0,  0, 0,            0 }
};
...
static void
apuinfo_list_add (unsigned short apui, unsigned short value)
{
  apuinfo_list *entry = head;

  while (entry != NULL)
    {
      if (entry->u.value[0] == apui && entry->u.value[1] == value)
	return;
      entry = entry->next;
    }
  ...
  entry->u.value[0] = apui;
  entry->u.value[1] = value;
  entry->next  = head;
  head = entry;
}

static apuinfo_list *
apuinfo_list_get_entry(unsigned short apui)
{
  apuinfo_list *entry = head;

  while (entry != NULL)
    {
      if (entry->u.value[0] == apui)
	return entry;
      entry = entry->next;
    }
  return NULL;
...
/* handle fixed register information for additional sdata sections */
/* fixed register set in the input bfds */
static unsigned int ppc_fixed_regs = 0;
/* fixed regs required by the linker */
static unsigned int ppc_fixed_regs_used = 0;
...
/* build string with fixed register names */
static void 
ppc_fixed_regs_to_string(unsigned short fixed_regs, char *str)
{
  unsigned int regno = 14;
  int comma = 0;
  char *ptr;
  ...
  for (ibfd = link_info->input_bfds; ibfd; ibfd = ibfd->link_next)
    {
      unsigned long datum;
      char *ptr;
      bfd_boolean apu_fixed_found = FALSE;

      asec = bfd_get_section_by_name (ibfd, APUINFO_SECTION_NAME);
      if (asec == NULL)
        {
          ppc_fixed_regs = 0;
          apuinfo_list_add (PPC_APUINFO_FIXED_REGS,0);
          continue;
        }
	...
	goto fail;
      ...
      for (i = 0; i < datum; i += 4)
        {
          unsigned short apui = bfd_get_16(ibfd, ptr + 20 + i);
          unsigned short value = bfd_get_16(ibfd, ptr + 20 + i + 2);

          /* check for fixed regs */
          if (apui == PPC_APUINFO_FIXED_REGS)
            {
#ifdef DEBUG
              fprintf(stderr,"%s: fixed = %x\n",ibfd->filename,value);
#endif
              apuinfo_list *entry = apuinfo_list_get_entry(apui);
              if (entry)
                {
                  entry->u.value[1] &= value;
                  ppc_fixed_regs &= value;
                }
              else
                {
                  apuinfo_list_add (apui,value);
                  ppc_fixed_regs = value;
                }
              apu_fixed_found = TRUE;
            }
          else
            apuinfo_list_add (apui,value);
        }
      if (apu_fixed_found == FALSE)
        {
          apuinfo_list *entry =
           apuinfo_list_get_entry(PPC_APUINFO_FIXED_REGS);
          if (entry)
            {
              entry->u.value[1] = 0;
            }
...
#if 0
          char fixed_regs[128];
          ppc_fixed_regs_to_string(ppc_fixed_regs, fixed_regs);
          /* FIXME: should we emit an error here or simple handle it as
             an APU with fixed regs = 0 */ 
          (*_bfd_error_handler)(_("%B: no APUINFO for fixed registers defined. Fixed registers are '%s'"),ibfd,fixed_regs); 
#endif
          ppc_fixed_regs = 0;
        }
#ifdef DEBUG
      fprintf(stderr,"%s fixed = %x\n",ibfd->filename,ppc_fixed_regs);
#endif
      apu_fixed_found = FALSE;
...
#ifdef DEBUG
  fprintf(stderr,"FIXED registers from input objects = %x\n",ppc_fixed_regs);
#endif
...
ppc_elf_final_write_processing (bfd *abfd, bfd_boolean linker ATTRIBUTE_UNUSED)
{
  ...
  for (i = 0; i < num_entries; i++)
    {
      apuinfo_list *entry = apuinfo_list_element (i);
      bfd_put_16 (abfd, entry->u.value[0], buffer + length);
      length += 2;
      bfd_put_16 (abfd, entry->u.value[1], buffer + length);
      length += 2;
    }
...
/*
   check fixed register definition in all input bfds 
   against the final use for SDAs
*/
static
bfd_boolean ppc_final_fixed_reg_check(struct bfd_link_info *link_info)
{
  const char *error_message = NULL;
  bfd *ibfd;
  unsigned long	length;
  ...
  bfd_byte *buffer;
  apuinfo_list *entry;
  bfd_boolean has_fixed_regs = FALSE;
...
#endif
  if (ppc_fixed_regs_used && 
      (ppc_fixed_regs == 0 || ((ppc_fixed_regs_used & ppc_fixed_regs) != ppc_fixed_regs_used)))
    {
      /* the executable uses special SDA sections check all input bfds 
         for using of fixed registers
      */
      entry = apuinfo_list_get_entry(PPC_APUINFO_FIXED_REGS);
      if (entry == NULL)
        {
          (*_bfd_error_handler)(_("no APUINFO for fixed registers defined"));
          return FALSE;
        }
      length = apuinfo_list_length ();
      length = 20 + length * 4;
      buffer = bfd_malloc (length);
      if (buffer == NULL)
        {
      ...
      for (ibfd = link_info->input_bfds; ibfd; ibfd = ibfd->link_next)
        {
          ...
          has_fixed_regs = FALSE;
          isec = bfd_get_section_by_name (ibfd, APUINFO_SECTION_NAME);
          if (isec == NULL)
            {
              char fixed_regs[128];
              ppc_fixed_regs_to_string(ppc_fixed_regs_used, fixed_regs);

              (*_bfd_error_handler)(_("%B: no fixed registers set, need '%s'"),ibfd,fixed_regs);
              check_ok = FALSE;
              continue;
            }
          length = isec->size;
          ...
          if (bfd_seek (ibfd, isec->filepos, SEEK_SET) != 0
          ...
          for (i = 0; i < datum; i += 4)
            {
              ...
              if (apui == PPC_APUINFO_FIXED_REGS)
                {
                  has_fixed_regs = TRUE;
                  if ((ppc_fixed_regs_used & value) != ppc_fixed_regs_used)
                    {
                      char ibfd_fixed_regs[128];
                      char fixed_regs[128];
                      ppc_fixed_regs_to_string(value, ibfd_fixed_regs);
                      ppc_fixed_regs_to_string(ppc_fixed_regs_used, fixed_regs);
                      (*_bfd_error_handler)(_("%B: has not the required fixed registers set"
                                              " '%s' <> need '%s'")
                                            ,ibfd,ibfd_fixed_regs,fixed_regs);
                      check_ok = FALSE;
                    }
                }
            }
          if (ppc_fixed_regs_used && (has_fixed_regs == FALSE))
            {
              char fixed_regs[128];
              ppc_fixed_regs_to_string(ppc_fixed_regs_used, fixed_regs);
              (*_bfd_error_handler)(_("%B: no fixed registers set, need '%s'"),ibfd,fixed_regs);
...
/* the number of sda sections 
   we have 6 of them
   .sdata, .sdata2. .sdata14, .sdata15, .sdata16, .sdata17
*/
...
struct ppc_elf_link_hash_table
{
  ...
  asection *dynsbss;
  asection *relsbss;
  elf_linker_section_t sdata[MAX_SDAS];
  asection *sbss;
...
ppc_elf_link_hash_table_create (bfd *abfd)
{
  ...
  ret->sdata[2].name = ".sdata14";
  ret->sdata[2].sym_name = "_SDA14_BASE_";
  ret->sdata[2].bss_name = ".sbss14";
  ret->sdata[2].reg = 14;

  ret->sdata[3].name = ".sdata15";
  ret->sdata[3].sym_name = "_SDA15_BASE_";
  ret->sdata[3].bss_name = ".sbss15";
  ret->sdata[3].reg = 15;

  ret->sdata[4].name = ".sdata16";
  ret->sdata[4].sym_name = "_SDA16_BASE_";
  ret->sdata[4].bss_name = ".sbss16";
  ret->sdata[4].reg = 16;

  ret->sdata[5].name = ".sdata17";
  ret->sdata[5].sym_name = "_SDA17_BASE_";
  ret->sdata[5].bss_name = ".sbss17";
  ret->sdata[5].reg = 17;
  ...
  if (sym->st_shndx == SHN_COMMON
      ...
      && sym->st_size <= elf_gp_size (abfd))
    {
	 ...
	 put into .sbss.  */
      ...
      if (htab->sbss == NULL)
	{
	  flagword flags = SEC_IS_COMMON | SEC_LINKER_CREATED | SEC_ALLOC;
...
#ifdef HTC_PPCVLE
          || r_type == R_PPC_VLE_REL8 
          || r_type == R_PPC_VLE_REL15
		      ...
		      const Elf_Internal_Rela *relocs)
{
  ...
  for (rel = relocs; rel < rel_end; rel++)
	...
	case R_PPC_EMB_RELSDA:
        ...
        case R_PPC_VLE_SDAREL_HA16D:
#endif
            {
              ...
              for (i = 0; i < MAX_SDAS; i++)
                {
                  if (htab->sdata[i].sym == NULL
                      && !create_sdata_sym (info, &htab->sdata[i]))
                    return FALSE;
                }
              if (h != NULL)
                {
	...
	case R_PPC_VLE_RELAX32:
	case R_PPC_VLE_RELAX32PC:
	case R_PPC_VLE_RELAX32_PLT:
	case R_PPC_VLE_RELAX32PC_PLT:
	  break;
...
/* Relaxation trampolines.  r12 is available for clobbering (r11, is
   used for some functions that are allowed to break the ABI).  */
static const int shared_vle_stub_entry[] =
  {
    0x7d8802a6, /* mflr r12 */
    0xe9010080, /* se_bl .Lxxx, se_mflr r0 */
    0x70009000, /* e_add2is r0, (xxx-.Lxxx)@ha */
    0x1c000000, /* e_add16i r0, r0, (xxx-.Lxxx)@l */
    0x7d8803a6, /* mtlr r12 */
    0x00b00006, /* se_mtctr r0, se_bctr */
  };
/* Relaxation trampolines in VLE mode.  r0 is available for clobbering */
static const int vle_stub_entry[] =
  {
    0x7000e000, /* e_lis r0,xxx@ha */
    0x1c000000, /* e_add16i r0,r0,xxx@l */
    0x00b00006, /* se_mtctr r0, se_bctr */
  };
#endif /* HTC_PPCVLE */
...
//#ifdef HTC_PPCVLE
  vle_mode = (isec->flags & SHF_PPC_VLE)? TRUE : FALSE;
  /* in VLE mode we have 2 byte insns and 2 byte alignment */
  insn_size = (vle_mode)? 2 : 4;

  trampoff = (isec->size + (insn_size - 1)) & (bfd_vma) -insn_size;
...
//#endif
  ...
  for (irel = internal_relocs; irel < irelend; irel++)
	...
	case R_PPC_REL14_BRNTAKEN:
	  ...
	  break;
        ...
        case R_PPC_VLE_REL15:
          ...
          break;
        case R_PPC_VLE_REL24:
          /* In VLE mode we can only ignore the lowest bit,
             since each instruction can begin on an even address
             */
          max_branch_offset = 1 << 24;
          break;
#endif
      ...
      if (f == NULL)
	{
	  ...
	  if (link_info->shared)
	    {
              size = (vle_mode) ? ARRAY_SIZE(shared_vle_stub_entry) : ARRAY_SIZE(shared_stub_entry);
	      size *= 4;
	      insn_offset = (vle_mode)? 8 : 12;
	      stub_rtype = (vle_mode)? R_PPC_VLE_RELAX32PC: R_PPC_RELAX32PC;
	    }
	  else
	    {
//#ifdef HTC_PPCVLE
              size = (vle_mode) ? ARRAY_SIZE(vle_stub_entry) : ARRAY_SIZE(stub_entry);
	      size *= 4;
//#endif
	      insn_offset = 0;
	      stub_rtype = (vle_mode) ? R_PPC_VLE_RELAX32 : R_PPC_RELAX32;
	    }

	  if ((R_PPC_RELAX32_PLT - R_PPC_RELAX32
               != R_PPC_RELAX32PC_PLT - R_PPC_RELAX32PC)
              && (R_PPC_RELAX32_PLT - R_PPC_RELAX32
                  != R_PPC_VLE_RELAX32PC_PLT - R_PPC_VLE_RELAX32PC))
	    abort ();
	  if (tsec == htab->plt
	      || tsec == htab->glink)
	...
	case R_PPC_REL14_BRNTAKEN:
	  ...
	  break;
        ...
        {
          unsigned short t16;
	  t16 = bfd_get_16 (abfd, hit_addr);
	  t16 &= ~0xff;
	  t16 |= val & 0xff;
	  bfd_put_16 (abfd, t16, hit_addr);
	  break;
        }
...
//#ifdef HTC_PPCVLE
      isec->size = (isec->size + (insn_size - 1)) & (bfd_vma) -insn_size;
      /* Branch around the trampolines. In VLE it is always a short jump */
      if (vle_mode)
        val = SE_B + ((trampoff - isec->size) >> 1);
      else
        val = B + trampoff - isec->size;
      dest = contents + isec->size;
      isec->size = trampoff;
      if (insn_size == 2)
        bfd_put_16 (abfd, val, dest);
      else
...
//#endif
      ...
      if (link_info->shared)
        {
          stub = (vle_mode)? shared_vle_stub_entry : shared_stub_entry;
          size = (vle_mode)? ARRAY_SIZE(shared_vle_stub_entry) : ARRAY_SIZE(shared_stub_entry);
        }
      else
        {
//#ifdef HTC_PPCVLE
          stub = (vle_mode)? vle_stub_entry : stub_entry;
          size = (vle_mode)? ARRAY_SIZE(vle_stub_entry) : ARRAY_SIZE(stub_entry);
//#endif
        }
  ...
  *again = changes != 0;
  if (!*again && link_info->relocatable)
	...
	if ((ELF32_R_TYPE (irel->r_info) == R_PPC_RELAX32)
            || (ELF32_R_TYPE (irel->r_info) == R_PPC_VLE_RELAX32))
	  {
...
//#ifdef HTC_PPCVLE
              irel[0].r_info = (vle_mode)?
               ELF32_R_INFO (r_symndx, R_PPC_VLE_HA16A)
               : ELF32_R_INFO (r_symndx, R_PPC_ADDR16_HA);
//#endif /* HTC_PPCVLE */
	    irel[1].r_offset += 4;
			  ...
			  asection **local_sections)
{
  ...
  for (; rel < relend; rel++)
	...
	case R_PPC_VLE_RELAX32_PLT:
	  ...
	  if ((r_type == R_PPC_RELAX32_PLT) || (r_type == R_PPC_VLE_RELAX32_PLT))
	    goto relax32;
	  /* Fall thru */
...
#ifdef HTC_PPCVLE
	    /* t0 is HA OR VLE_HA16A, t1 is LO */
	    relocation += addend;

            if (vle_mode)
              {
                bfd_vma reloc_high = (relocation + 0x8000) >> 16;

                /* clear the operand bits */
                t0 &= ~0x001f07ff;
                t0 |= ((((reloc_high) & 0xf800) << 5) | ((reloc_high) & 0x7ff));
              }
            else
              {
                /* clear the operand bits */
...
#ifdef HTC_PPCVLE
            if (vle_mode)
              rel[0].r_info = ELF32_R_INFO (r_symndx, R_PPC_VLE_HA16A);
            else
#endif
...
#ifdef HTC_SUPPORT
              {
                const char *sd = strstr(name,".sdata");
                const char *sb = strstr(name,".sbss");
                if ((sd && (sd[6] == 0 || sd[6] == '.'))
                    || (sb && (sb[5] == 0 || sb[5] == '.')))
                  {
                    (*_bfd_error_handler)
	...
	case R_PPC_EMB_RELSDA:
...

	  {
	    ...
	    name = bfd_get_section_name (abfd, sec->output_section);
	    if (strcmp (name, ".PPC.EMB.sdata0") == 0
		...
		reg = 0;
              ...
              {
                for (i = MAX_SDAS-1; i >= 0; i--)
                  {
                    const char *sd = strstr (name, htab->sdata[i].name);
                    const char *sb = strstr (name, htab->sdata[i].bss_name);
                    const int sd_len = strlen(htab->sdata[i].name);
                    const int sb_len = strlen(htab->sdata[i].bss_name);
                    if ((sd && (sd[sd_len] == 0 || sd[sd_len] == '.'))
                        || (sb && (sb[sb_len] == 0 || sb[sb_len] == '.')))
                      {
                        sda = htab->sdata[i].sym;
                        reg = htab->sdata[i].reg;
                        break;
                      }
                  }
                if (sda == NULL)
                ...
                if (reg >= 14 && reg < 24)
                  {
                    ppc_fixed_regs_used |= (1 << (reg-14));

                    if (!(ppc_fixed_regs & (1 << (reg-14))))
                      {
                        (*_bfd_error_handler)
                         (_("error: %B 'r%d' is not a fixed register in all objects"),
                          input_bfd,
                          reg);
                      }
                  }
		...
		bfd_put_32 (output_bfd, insn, contents + rel->r_offset);
            ...
            if (r_type == R_PPC_VLE_SDA21 || r_type == R_PPC_VLE_SDA21_LO)
              {
                unsigned int sign = 0;

                insn = bfd_get_32 (output_bfd, contents + rel->r_offset);

                if (((insn & 0xfc000000) != 0x1c000000))
                  {
                    (*_bfd_error_handler)
                     (_("%B: relocation %s is only supported for \"e_add16i\" yet."),
                ...
                if (reg == 0)
                  {
                    /* use split20 format
                       change e_add16i ra,r0,offset to e_li ra,offset
                       */
                    if (value & 0x8000)
                      {
                        sign = 0xffffffff;
                    ...
                    insn |= 0x70000000
                     | ((sign & 10) << 15)          /* bit 11 */
                     | ((sign & 0xf) << 11)         /* bit 17-20 */
                     | ((value & 0xf000) << 4)      /* bit 12-15 */
                     | ((value & 0xfff) );          /* bit 21-31 */
                    bfd_put_32 (output_bfd, insn, contents + rel->r_offset);
                    continue;
                  }
                else
                  {
                    /* insert base regsiter and use reloc R_PPC_EMB_SDA21 */
            ...
            switch (r_type)
              ...
              case R_PPC_VLE_SDAREL_HA16A:
                /* use split16a */
                mask = 0x001f07ff;
                value = ((value & 0xf800) << 5) | (value & 0x7ff);
                break;
...
static bfd_boolean
ppc_elf_final_link (bfd *abfd, struct bfd_link_info *info)
{
  /* Invoke the regular ELF backend linker to do all the work.  */
  if (!bfd_elf_final_link (abfd, info))
    return FALSE;
  if (!ppc_final_fixed_reg_check(info))
    return FALSE;
  return TRUE;
}
