...
_bfd_elf_print_private_bfd_data (bfd *abfd, void *farg)
{
  ...
  if (p != NULL)
      ...
      for (i = 0; i < c; i++, p++)
	{
	  ...
	  fprintf (f, " flags %c%c%c%c",
		   (p->p_flags & PF_R) != 0 ? 'r' : '-',
		   (p->p_flags & PF_W) != 0 ? 'w' : '-',
		   (p->p_flags & PF_X) != 0 ? 'x' : '-',
		   (p->p_flags & PF_PPC_VLE) != 0 ? 'v' : '-');
	  if ((p->p_flags &~ (unsigned) (PF_R | PF_W | PF_X | PF_PPC_VLE)) != 0)
	    fprintf (f, " %lx", p->p_flags &~ (unsigned) (PF_R | PF_W | PF_X | PF_PPC_VLE));
#else
...
void
xml_bfd_elf_program_header (bfd *abfd, p_xml_element parent)
{
  ...
  if (p != NULL)
    {
      unsigned int i, c, j;
      p_xml_element program_headers = create_xml_element (XML_E_PROGRAM_HEADER_LIST);
      add_xml_child (parent, program_headers);
      ...
      for (i = 0; i < c; i++, p++)
        {
          const char *pt = get_segment_type (p->p_type);
          char buf[20];
          p_xml_element program_header = create_xml_element (XML_E_PROGRAM_HEADER);
          add_xml_child (program_headers, program_header);
          ...
          add_xml_attribute (program_header, XML_A_TYPE, pt);
          add_xml_attribute_hex (program_header, XML_A_OFFSET, p->p_offset);
          add_xml_attribute_hex (program_header, XML_A_VIRTUAL_ADDRESS, p->p_vaddr);
          add_xml_attribute_hex (program_header, XML_A_PHYSICAL_ADDRESS, p->p_paddr);

          sprintf (buf, "2**%u", bfd_log2 (p->p_align));
          add_xml_attribute (program_header, XML_A_ALIGN, buf);
          add_xml_attribute_hex (program_header, XML_A_FILE_SIZE, p->p_filesz);
          add_xml_attribute_hex (program_header, XML_A_MEMORY_SIZE, p->p_memsz);

          p_xml_element flags = create_xml_element (XML_E_FLAGS);
          add_xml_child (program_header, flags);

          for (j = 0; flag_list[j].flag != 0; ++j)
            {
              if (p->p_flags & flag_list[j].flag)
                {
                  p_xml_element flag = create_xml_element (XML_E_FLAG);
                  add_xml_value (flag, flag_list[j].name);
...
/* We don't support dynamic sections yet. Is should be kept in comment for the
   future. */
#if 0
bfd_boolean
xml_bfd_elf_dynamic_sections (bfd *abfd, p_xml_element parent)
{
  asection *s;
  bfd_byte *dynbuf = NULL;
  p_xml_element dyn_sections;
  p_xml_element dyn_section;
  ...
  if (s != NULL)
    {
      ...
      size_t extdynsize;
      void (*swap_dyn_in) (bfd *, const void *, Elf_Internal_Dyn *);
      dyn_sections = create_xml_element (XML_E_DYNAMIC_SECTION_LIST);

      add_xml_child (parent, dyn_sections);
      ...
      for (; extdyn < extdynend; extdyn += extdynsize)
        {
          ...
          dyn_section = create_xml_element (XML_E_DYNAMIC_SECTION);
          ...
          add_xml_attribute (dyn_section, XML_A_NAME, name);

          if (! stringp)
            {
              add_xml_attribute_hex (dyn_section, XML_A_VALUE, dyn.d_un.d_val);
            }
          else
            {
              const char *string;
              ...
              add_xml_attribute (dyn_section, XML_A_VALUE, string);
            }
        }
...
bfd_boolean
xml_bfd_elf_version_definitions (bfd *abfd, p_xml_element parent)
{
  if (elf_dynverdef (abfd) != 0 && elf_tdata (abfd)->verdef == NULL)
    {
      if (! _bfd_elf_slurp_version_tables (abfd, FALSE))
      ...
      if (elf_dynverdef (abfd) != 0)
        {
          Elf_Internal_Verdef *t;
          p_xml_element version_defs = create_xml_element (XML_E_VERSION_DEFINITION_LIST);

          add_xml_child (parent, version_defs);

          for (t = elf_tdata (abfd)->verdef; t != NULL; t = t->vd_nextdef)
            {
              p_xml_element version_def = create_xml_element (XML_E_VERSION_DEFINITION);

              add_xml_child (version_defs, version_def);
              add_xml_attribute_dec (version_def, XML_A_INDEX, t->vd_ndx);
              add_xml_attribute_hex (version_def, XML_A_FLAGS, t->vd_flags);
              add_xml_attribute_hex (version_def, XML_A_HASH, t->vd_hash);
              add_xml_attribute (version_def, XML_A_NODENAME, t->vd_nodename ? t->vd_nodename : "<corrupt>");
              ...
              if (t->vd_auxptr != NULL && t->vd_auxptr->vda_nextptr != NULL)
                {
                  ...
                  for (a = t->vd_auxptr->vda_nextptr;
                       ...
                       a = a->vda_nextptr)
                    {
                      p_xml_element node = create_xml_element (XML_E_NODENAME);
                      add_xml_child (version_def, node);
                      add_xml_value (node, a->vda_nodename ? a->vda_nodename : "<corrupt>");
                    }
                }
            }
        }
...
bfd_boolean
xml_bfd_elf_version_references (bfd *abfd, p_xml_element parent)
{
  if (elf_dynverref (abfd) != 0 && elf_tdata (abfd)->verref == NULL)
    {
      if (! _bfd_elf_slurp_version_tables (abfd, FALSE))
        return FALSE;
      ...
      if (elf_dynverref (abfd) != 0)
        {
          Elf_Internal_Verneed *t;
          p_xml_element ver_refs = create_xml_element (XML_E_VERSION_REFERENCE_LIST);
          add_xml_child (parent, ver_refs);

          for (t = elf_tdata (abfd)->verref; t != NULL; t = t->vn_nextref)
            {
              Elf_Internal_Vernaux *a;
              p_xml_element req = create_xml_element (XML_E_REQUIRED_FROM);
              add_xml_child (ver_refs, req);
              add_xml_attribute (req, XML_A_FILENAME, t->vn_filename ? t->vn_filename : "<corrupt>");

              for (a = t->vn_auxptr; a != NULL; a = a->vna_nextptr)
                {
                  p_xml_element ref = create_xml_element (XML_E_VERSION_REFERENCE);
                  add_xml_child (req, ref);
                  add_xml_attribute_hex (ref, XML_A_HASH, a->vna_hash);
                  add_xml_attribute_hex (ref, XML_A_FLAGS, a->vna_flags);
                  add_xml_attribute_dec (ref, XML_A_OTHER, a->vna_other);
                  add_xml_attribute (ref, XML_A_NODENAME, a->vna_nodename ? a->vna_nodename : "<corrupt>");
                }
            }
        }
    }
...
prep_headers (bfd *abfd)
{
  ...
  Elf_Internal_Phdr *i_phdrp ATTRIBUTE_UNUSED = 0;/* Program header table, internal form */
...
{
  const struct elf_backend_data *bed = get_elf_backend_data (abfd);
  Elf_Internal_Ehdr *i_ehdrp ATTRIBUTE_UNUSED;
  Elf_Internal_Shdr **i_shdrp;
  bfd_boolean failed;
...
#ifdef HTC_SUPPORT
flagword
_bfd_elf_get_default_section_flags(bfd *abfd,asection *sec)
{
  const struct elf_backend_data *bed;
  const struct bfd_elf_special_section *ssect;
  flagword flags = 0;
  ...
  if (ssect != NULL)
    {
      ...
      if (ssect->attr & SHF_ALLOC)
        flags |= SEC_ALLOC;
      if (ssect->attr & SHF_EXECINSTR)
        flags |= SEC_CODE;
      if (!(ssect->attr & SHF_WRITE))
        flags |= SEC_READONLY;
      else
        flags &= ~SEC_READONLY;
      bed->elf_backend_section_flags(&flags,&elf_section_data (sec)->this_hdr);
    }
  return flags;
}
#endif /* HTC_PPCVLE */
