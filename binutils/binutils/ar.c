...
usage (int help)
{
  ...
  if (! is_ranlib)
	...
	_("Usage: %s %s[emulation options] [--plugin <name>] [-]{dmpqrstx}[abcfilNoPsSuvV] [member-name] [count] archive-file file...\n");
#else
	_("Usage: %s %s[emulation options] [-]{dmpqrstx}[abcfilNoPsSuvV] [member-name] [count] archive-file file...\n");
#endif
      ...
      /* xgettext:c-format */
#ifdef HTC_SUPPORT
      fprintf (s, _("       %s [--error-numbers] -M [<mri-script]\n"), program_name);
#else
...
#if defined (BFD_SUPPORTS_PLUGIN) || defined (HTC_SUPPORT)
      fprintf (s, _(" optional:\n"));
#endif
#ifdef HTC_SUPPORT
      fprintf (s, _("  --error-numbers - Add unique numbers for each error and warning message.\n"));
#endif
#if BFD_SUPPORTS_PLUGINS
  ...
  else
    {
      /* xgettext:c-format */
#ifdef HTC_SUPPORT
      fprintf (s, _("Usage: %s [--error-numbers] [options] archive\n"), program_name);
#else
...
#ifdef HTC_SUPPORT
  if (argc >= 2 && strcmp (argv[1], "--error-numbers") == 0)
    {
      set_error_numbering();
      argv[1] = argv[0];
      argv++;
