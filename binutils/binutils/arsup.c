  ...
  if (!obfd)
      ...
      if (!t)
	{
	  ...
	  ibfd = bfd_openr (name, NULL);
          ...
          /* remember the input archive to execute a close before
             renaming (necessary for Windows) */
          input_arch = ibfd;
...
ar_save (void)
{
  if (!obfd)
  ...
  else
    {
      ...
      /* close the original input archive because
         we will override it with the newly created
         (necessary for Windows to allow remove of the
         original archive */
      if (input_arch)
        {
          bfd_close(input_arch);
          input_arch = 0;
        }
