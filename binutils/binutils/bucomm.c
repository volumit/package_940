...
#ifdef HTC_SUPPORT
void
xml_arelt_descr (p_xml_element parent, bfd *abfd, bfd_boolean verbose)
{
  struct stat buf;
  p_xml_element archive = create_xml_element (XML_E_ARCHIVE_HEADER);
  add_xml_child (parent, archive);
  ...
  if (verbose)
      ...
      if (bfd_stat_arch_elt (abfd, &buf) == 0)
	{
	  ...
	  /* POSIX 1003.2/D11 says to skip first character (entry type).  */
          add_xml_attribute (archive, XML_A_MODE, modebuf + 1);
          add_xml_attribute_dec (archive, XML_A_UID, (long) buf.st_uid);
          add_xml_attribute_dec (archive, XML_A_GID, (long) buf.st_gid);
          add_xml_attribute_dec (archive, XML_A_SIZE, (long) buf.st_size);
          add_xml_attribute (archive, XML_A_TIME, timebuf);
	}
    }

  add_xml_attribute (archive, XML_A_FILENAME, bfd_get_filename (abfd));
}
#endif
